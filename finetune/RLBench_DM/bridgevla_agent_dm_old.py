...
class HeatmapInferenceMVRotGrip:
    ...
    def __init__(self,
        ...
        if wan_type == "5B_TI2V_RGB_HEATMAP_MV_ROT_GRIP":
            from diffsynth.pipelines.wan_video_5B_TI2V_heatmap_and_rgb_mv_view import WanVideoPipeline
            print("  Loading single-frame pipeline: wan_video_5B_TI2V_heatmap_and_rgb_mv_view")
        ...
        # 初始化旋转和夹爪预测器（View拼接版本 - 直接使用VAE latents）
        print("Initializing rotation and gripper predictor (View concatenation + direct latent version)...")
        print(f"  use_initial_gripper_state: {use_initial_gripper_state}")
        print(f"  Using VIEW concatenation: 6 views (3 RGB + 3 Heatmap)")
        print(f"  Direct latent input: 48 channels (not 256 decoder features)")
