...
def main(argv):
    ...
    # Validate image_size has exactly 2 elements
    if len(FLAGS.image_size) != 2:
        raise ValueError(
            f'image_size must have exactly 2 elements (width, height), '
            f'got {len(FLAGS.image_size)}: {FLAGS.image_size}. '
            f'Use comma-separated format: --image_size 256,256'
        )
