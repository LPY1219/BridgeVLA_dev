...
class MVT(nn.Module):
    def __init__(
        ...
        self.up0 = ConvexUpSample(
            in_dim=self.vlm_dim,
            out_dim=self.num_local_point,  # 输出多通道heatmap
        ...
        import os
        if os.path.exists("/share/project/lxh/project/VLMs/paligemma-3b-pt-224"):
        ...
        if not forward_no_feat:
            ...
            if not self.training:
                wpt_local = self.get_wpt(
                    ...
                    dyn_cam_info=None,
                ) # bs,num_local_point,3

            # projection
            # (bs, num_local_point, num_img, 2)
