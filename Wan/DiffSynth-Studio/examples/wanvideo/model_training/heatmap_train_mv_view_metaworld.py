...
from diffsynth.trainers.heatmap_dataset_mv_with_rot_grip_3cam_metaworld import HeatmapDatasetFactory
# 多帧历史支持的数据集和工厂
        ...
        for epoch_id in range(num_epochs):
            ...
            for step, data in enumerate(pbar):
                with accelerator.accumulate(model):
                    ...
                    # 前向传播
                    try:
                        _load_from_cache = getattr(dataset, "load_from_cache", False)
                        if not _load_from_cache and hasattr(dataset, "datasets"):
                            for _ds in dataset.datasets:
                                if getattr(_ds, "load_from_cache", False):
                                    _load_from_cache = True 
                                    break
                        if _load_from_cache:
...
if __name__ == "__main__":
    ...
    import os
    local_rank = int(os.environ.get("LOCAL_RANK", 0))
    # 使用全局RANK来判断主进程（多机训练时每个节点都有LOCAL_RANK=0）
    global_rank = int(os.environ.get("RANK", 0))
    is_main_process = global_rank == 0  # 只有全局RANK=0才是主进程
