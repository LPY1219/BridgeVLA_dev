...
class MultiViewRotationGripperPredictor(nn.Module):
    ...
    def forward(
        ...
        combined_features = combined_features.view(b * v, c_total, t_upsampled, h, w)
        features = self.feature_extractor(combined_features)  # (b*v, hidden_dim, t, 1, 1) # 可能有些问题
...
def decode_latents_to_features(rgb_latents, heatmap_latents, vae, device):
    """
...
def train_epoch(
    ...
    for step, batch in enumerate(pbar):
        with accelerator.accumulate(model):
            ...
            # 在训练循环中decode latent到features（每个GPU并行decode）
            ...
            rgb_features, heatmap_features = decode_latents_to_features(
                rgb_latents, heatmap_latents, vae, accelerator.device
            )
...
def main():
    ...
    if len(data_roots) > 1 or not use_cached_latents:
        ...
        # 如果是单任务且需要预计算缓存
        if len(data_roots) == 1 and not use_cached_latents:
            print("\n" + "="*60)
