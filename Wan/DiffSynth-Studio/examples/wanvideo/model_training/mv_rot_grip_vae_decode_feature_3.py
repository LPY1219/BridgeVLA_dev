...
class MultiViewRotationGripperPredictor(nn.Module):
    """
    预测旋转和夹爪变化量的模型 - 使用VAE Decoder的中间上采样特征
...
class VAEFeatureExtractor:
    ...
    def _encode_and_decode_single_view_with_history(self, video_frames, num_history_frames, tiled, tile_size, tile_stride, apply_heatmap_scale=False, heatmap_latent_scale=1.0, latent_noise_std=0.0):
        """
        根据历史帧数量编码并解码单个视角的视频（参考 wan_video_5B_TI2V_heatmap_and_rgb_mv_history.py）

        关键：当 num_history_frames=2 时，需要分别编码和解码，确保输出帧数与输入帧数一致。
        - 第1帧单独编码解码，输出1帧
        - 第2帧+剩余帧正常编码解码，输出(n-1)帧
        ...
        Returns:
            ...
            其中 t_upsampled 与 len(video_frames) 相等

        编码规则：
        - num_history_frames=1: 正常编码解码
        - num_history_frames=2: 第1帧单独编码解码，第2帧+剩余帧作为新视频正常编码解码，然后concat
        - num_history_frames=1+4N: 正常编码解码
        """
        num_frames = len(video_frames)

        if num_history_frames == 2 and num_frames >= 2:
            # 特殊处理：第1帧单独编码解码，第2帧+剩余帧正常编码解码
            # Frame 0 单独编码
            ...
            z_0 = z_0.to(dtype=self.torch_dtype, device=self.device)
            ...
            # 解码第一帧的latent -> 输出1帧
            feat_0 = self.vae.decode_intermediate(z_0, device=self.device, tiled=False)
            feat_0 = feat_0[0].to(dtype=self.torch_dtype, device=self.device)  # (c, t=1, h, w)

            # Frame 1 + 剩余帧作为新视频正常编码
            remaining_frames = video_frames[1:]  # 从第2帧开始（共num_frames-1帧）
            ...
            z_remaining = z_remaining.to(dtype=self.torch_dtype, device=self.device)
            ...
            # 解码剩余帧的latent -> 输出(num_frames-1)帧
            feat_remaining = self.vae.decode_intermediate(z_remaining, device=self.device, tiled=False)
            feat_remaining = feat_remaining[0].to(dtype=self.torch_dtype, device=self.device)  # (c, t=num_frames-1, h, w)

            # 在时间维度上concat: [第1帧特征, 剩余帧特征]
            intermediate_features = torch.cat([feat_0, feat_remaining], dim=1)  # (c, t=num_frames, h, w)
            ...
            # 解码
            intermediate_features = self.vae.decode_intermediate(latents, device=self.device, tiled=False)
            intermediate_features = intermediate_features[0].to(dtype=self.torch_dtype, device=self.device)
    ...
    def encode_and_decode_intermediate(
        ...
        Args:
            ...
            heatmap_latent_scale: heatmap latent的缩放因子
            num_history_frames: 历史帧数量（用于特殊编码模式）
        ...
        Returns:
            ...
            其中:
            - c_intermediate = 256 (VAE decoder最后一个上采样块的输出通道数)
            - t_upsampled 是时间维度上采样后的长度（与原始帧数相等）
        ...
        for view_idx in range(num_views):
            ...
            # 使用支持history frames的编码和解码方法
            view_rgb_intermediate = self._encode_and_decode_single_view_with_history(
        ...
        for view_idx in range(num_views):
            ...
            # 使用支持history frames的编码和解码方法
            view_heatmap_intermediate = self._encode_and_decode_single_view_with_history(
