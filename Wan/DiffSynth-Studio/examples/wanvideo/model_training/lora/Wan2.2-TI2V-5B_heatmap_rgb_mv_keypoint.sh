...
# export CUDA_VISIBLE_DEVICES=0,2,3,4,5,6,7; NUM_GPUS=7
# 测试用4个GPU（推荐先用这个排查问题）
...
# 机器1的路径配置
MACHINE1_HEATMAP_DATA_ROOT="/DATA/disk1/lpy/Franka_data_3zed_2_keypoint/put_red_bull_in_pink_plate"
...
MACHINE2_HEATMAP_DATA_ROOT="/data/Franka_data_3zed_2_keypoint/put_red_bull_in_pink_plate"    # TODO: 填写当前机器的热力图数据根目录
...
# 机器3的路径配置
MACHINE3_HEATMAP_DATA_ROOT="/DATA/disk0/lpy/data/Franka_data_3zed_2_keypoint/put_red_bull_in_pink_plate"  # TODO: 根据实际任务修改
...
# 机器4的路径配置
MACHINE4_HEATMAP_DATA_ROOT="/DATA/disk2/lpy/data/Franka_data_3zed_2_keypoint/put_red_bull_in_pink_plate"  # TODO: 根据实际任务修改
...
LEARNING_RATE=4e-5
NUM_EPOCHS=5000                       # 最大训练epoch数
SAVE_EPOCHS_INTERVAL=500               # 每隔多少个epoch保存一次模型（0表示只在最后保存）
...
OUTPUT_PATH="${OUTPUT_BASE}/Wan2.2-TI2V-5B_heatmap_rgb_lora/8_trajectory_put_plate_1camera_keypoint_pretrain_${LOAD_PRETRAINED_CHECKPOINT}_unfreeze_modulate_${UNFREEZE_MODULATION_AND_NORMS}_differentproj_${USE_DIFFERENT_PROJECTION}/${TIMESTAMP}"
...
# 机器4的预训练checkpoint路径
MACHINE4_PRETRAINED_CHECKPOINT="/DATA/disk2/lpy/BridgeVLA_dev/logs/Wan/pretrain/Wan2.2-TI2V-5B_mvtrack_pretrain/20251206_075139/epoch-9.safetensors"
