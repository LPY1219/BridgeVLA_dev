...
#
# 特殊配置：冻结所有参数，只训练patch_embedding和head.head
...
MACHINE1_MODEL_BASE_PATH="/share/project/lpy/huggingface/Wan2.2-TI2V-5B-fused"
...
OUTPUT_PATH="${OUTPUT_BASE}/Wan2.2-TI2V-5B_heatmap_rgb_freeze_all/${TIMESTAMP}"
# 指定Wan模型类型
...
# ==============================================
# 特殊配置：冻结所有参数，只训练patch_embedding和head
# ==============================================
# 不配置trainable_models和lora_base_model参数
# heatmap_train.py会自动调用_unfreeze_patch_embedding_and_head()
# 只解冻patch_embedding和head的参数进行训练，其他参数保持冻结
# 注意：不需要设置TRAINABLE_MODULES和LORA_BASE_MODEL变量
...
SWANLAB_PROJECT="Wan2.2-TI2V-5B_heatmap_rgb_freeze_all"  # SwanLab项目名称
SWANLAB_EXPERIMENT="heatmap-freeze-all-TI2V-5B-$(date +%Y%m%d-%H%M%S)"  # SwanLab实验名称（添加时间戳）
DEBUG_MODE=false                            # 调试模式（为true时禁用SwanLab）

echo "================================================================"
echo "HEATMAP SEQUENCE TRAINING FOR Wan2.2-TI2V-5B (FREEZE ALL MODE)"
echo "================================================================"
...
echo "  训练模式: 全参数冻结，仅训练 patch_embedding 和 head"
...
# ================================================================
echo "================================================================"
echo "STARTING HEATMAP TRAINING (FREEZE ALL MODE)"
echo "================================================================"
