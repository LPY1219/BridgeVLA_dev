...
if [ -d "${MACHINE1_COPPELIASIM_ROOT}" ]; then
    ...
    # 确保conda环境的Python路径在PATH最前面（解决tmux中PATH顺序问题）
...
# 机器3的路径配置
MACHINE3_DATA_ROOT="/DATA/disk0/lpy/3D-Diffusion-Policy/3D-Diffusion-Policy/data/metaworld_door-open_expert.zarr"
...
# 初始夹爪状态输入配置
# true: 将初始夹爪状态作为模型输入（仅夹爪状态，不包括旋转）
# false: 不使用初始夹爪状态作为输入
...
SWANLAB_PROJECT="mv_rot_grip_metaworld"
...
if [ "${NUM_MACHINES}" -gt 1 ]; then
    ...
    accelerate launch \
        ...
        examples/wanvideo/model_training/mv_rot_grip_vae_decode_feature_3_view_metaworld.py \
        --data_root ${DATA_ROOT} \
