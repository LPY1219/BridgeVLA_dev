...
# ==============================================
# This script uses VIEW concatenation instead of CHANNEL concatenation
# - in_dim=48 (original pretrained dimension)
# - out_dim=48 (original pretrained dimension)
# - num_views=6 (3 RGB views + 3 Heatmap views)
# - Concatenates along batch/view dimension (dim=0)
# - No special weight adaptation needed for patch_embedding
# ==============================================
...
# 单机训练配置
# ==============================================
# 单机训练时设置这些环境变量，使Python脚本能够正确识别配置
...
echo "单机训练配置"
echo "================================================================"
echo "机器数量: ${NUM_MACHINES}"
echo "每机GPU数: ${NUM_GPUS_PER_NODE}"
echo "总GPU数: $((NUM_MACHINES * NUM_GPUS_PER_NODE))"
echo "================================================================"
echo ""
...
# 机器2的路径配置（待填写）
MACHINE2_HEATMAP_DATA_ROOT="/data/Franka_data_3zed_5/"    # TODO: 填写当前机器的热力图数据根目录
...
MACHINE2_MODEL_BASE_PATH="/data/lpy/huggingface/Wan2.2-TI2V-5B/Wan2.2-TI2V-5B"      # TODO: 填写当前机器的模型基础路径

# 机器3的路径配置
MACHINE3_HEATMAP_DATA_ROOT="/DATA/disk0/lpy/data/Franka_data_3zed_5/"  # TODO: 根据实际任务修改
...
# 机器4的路径配置
MACHINE4_HEATMAP_DATA_ROOT="/DATA/disk1/lpy_a100_4/data/Franka_data_3zed_5/"  # TODO: 根据实际任务修改
MACHINE4_OUTPUT_BASE="/DATA/disk1/lpy_a100_4/BridgeVLA_dev/logs/Wan/train"
MACHINE4_MODEL_BASE_PATH="/DATA/disk1/lpy_a100_4/huggingface/Wan2.2-TI2V-5B"  # View concat使用原始预训练模型
...
TRAIL_END="8"            # 结束trail编号，如50表示到trail_50结束。留空表示不限制
...
GRADIENT_ACCUMULATION_STEPS=2           # 增加梯度累积步数以保持相同的有效批次大小
...
NAME="view_concat_16cards_${LOAD_PRETRAINED_CHECKPOINT}_history_${NUM_HISTORY_FRAMES}_seq_${SEQUENCE_LENGTH}_new_projection"
OUTPUT_PATH="${OUTPUT_BASE}/Wan2.2-TI2V-5B_heatmap_rgb_lora_view/5_trajectory_pour_3camera_${NAME}_rgb_loss_${RGB_LOSS_WEIGHT}/${TIMESTAMP}"
