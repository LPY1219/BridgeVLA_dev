...
# ==============================================
# 在使用当前机器之前，请填写以下配置项：
# 1. MACHINE2_CONDA_PATH: conda安装路径
# 2. MACHINE2_CONDA_ENV: conda环境名称
# 3. MACHINE2_COPPELIASIM_ROOT: CoppeliaSim安装路径
# 4. MACHINE2_DISPLAY: DISPLAY环境变量
...
# 机器1的CoppeliaSim配置
MACHINE1_COPPELIASIM_ROOT="/share/project/lpy/BridgeVLA/finetune/CoppeliaSim_Edu_V4_1_0_Ubuntu20_04"
...
if [ "${CURRENT_MACHINE}" = "machine1" ]; then
    if [ -d "${MACHINE1_COPPELIASIM_ROOT}" ]; then
        ...
        echo "已设置机器1的CoppeliaSim环境变量"
    else
        echo "警告：机器1的CoppeliaSim路径不存在: ${MACHINE1_COPPELIASIM_ROOT}"
    fi
elif [ "${CURRENT_MACHINE}" = "machine2" ]; then
    if [ -d "${MACHINE2_COPPELIASIM_ROOT}" ]; then
        export COPPELIASIM_ROOT="${MACHINE2_COPPELIASIM_ROOT}"
        export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$COPPELIASIM_ROOT
        export QT_QPA_PLATFORM_PLUGIN_PATH=$COPPELIASIM_ROOT
        export DISPLAY="${MACHINE2_DISPLAY}"
        echo "已设置机器2的CoppeliaSim环境变量"
    else
        echo "警告：机器2的CoppeliaSim路径不存在: ${MACHINE2_COPPELIASIM_ROOT}"
    fi
fi
...
# 机器1的项目路径配置
MACHINE1_PROJECT_ROOT="/share/project/lpy/BridgeVLA/Wan/DiffSynth-Studio"
MACHINE1_DEEPSPEED_CONFIG_DIR="/share/project/lpy/BridgeVLA/Wan/DiffSynth-Studio/examples/wanvideo/model_training/config"
...
# DeepSpeed配置 - 使用ZeRO Stage 3 (更激进的显存优化)
ENABLE_DEEPSPEED=false                   # 是否启用DeepSpeed
DEEPSPEED_STAGE=2                       # ZeRO stage (2 或 3)
...
# 四GPU: 
export CUDA_VISIBLE_DEVICES=4,5,6,7; NUM_GPUS=4
...
# 机器1的路径配置
MACHINE1_HEATMAP_DATA_ROOT="/share/project/lpy/test/FA_DATA/data/filtered_data/put_the_lion_on_the_top_shelf"
MACHINE1_OUTPUT_BASE="/share/project/lpy/BridgeVLA/logs/Wan/train"
MACHINE1_MODEL_BASE_PATH="/share/project/lpy/huggingface/Wan2.1-I2V-14B-480P"

# 机器2的路径配置（待填写）
MACHINE2_HEATMAP_DATA_ROOT="/data/wxn/V2W_Real/put_the_lion_on_the_top_shelf"    # TODO: 填写当前机器的热力图数据根目录
MACHINE2_OUTPUT_BASE="/home/lpy/BridgeVLA_dev/logs/Wan/train"          # TODO: 填写当前机器的输出基础目录
MACHINE2_MODEL_BASE_PATH="/data/lpy/huggingface/Wan2.1-I2V-14B-480P"      # TODO: 填写当前机器的模型基础路径
...
OUTPUT_PATH="${OUTPUT_BASE}/Wan2.1-I2V-14B-480P_heatmap_lora/${TIMESTAMP}"
# 指定Wan模型类型
WAN_TYPE="WAN_2_1_14B_I2V"

# 热力图专用参数
SEQUENCE_LENGTH=4          # 热力图序列长度 # 热力图序列加上原始图像的得到的总帧数，必须除以4余一
...
echo "Total effective batch size: ${EFFECTIVE_BATCH_SIZE} (2 GPUs × 1 batch × 8 accumulation = 16)"
...
PREFETCH_FACTOR=1                       # 减少数据预取因子以节省显存
...
SWANLAB_PROJECT="wan2.1-heatmap-training-480P"  # SwanLab项目名称
SWANLAB_EXPERIMENT="heatmap-lora-I2V14B-480P-$(date +%Y%m%d-%H%M%S)"  # SwanLab实验名称（添加时间戳）
DEBUG_MODE=false                            # 调试模式（为true时禁用SwanLab）

echo "================================================================"
echo "HEATMAP SEQUENCE TRAINING FOR WAN2.1-I2V-14B-480P"
echo "================================================================"
...
echo "  DeepSpeed启用: ${ENABLE_DEEPSPEED}"
echo "  DeepSpeed阶段: ${DEEPSPEED_STAGE}"
...
# ================================================================
# 单阶段训练（Wan2.1-I2V-14B-480P不需要分阶段）
# ================================================================
echo "================================================================"
...
if [ "${ENABLE_DEEPSPEED}" = "true" ]; then
  echo "🚀 Using DeepSpeed with existing accelerate config"

  # 使用deepspeed配置文件
  accelerate launch \
    --config_file examples/wanvideo/model_training/lora/accelerate_config_gpu.yaml \
  ...
  --model_paths '[
    [
        "'${MODEL_BASE_PATH}'/diffusion_pytorch_model-00001-of-00007.safetensors",
        "'${MODEL_BASE_PATH}'/diffusion_pytorch_model-00002-of-00007.safetensors",
        "'${MODEL_BASE_PATH}'/diffusion_pytorch_model-00003-of-00007.safetensors",
        "'${MODEL_BASE_PATH}'/diffusion_pytorch_model-00004-of-00007.safetensors",
        "'${MODEL_BASE_PATH}'/diffusion_pytorch_model-00005-of-00007.safetensors",
        "'${MODEL_BASE_PATH}'/diffusion_pytorch_model-00006-of-00007.safetensors",
        "'${MODEL_BASE_PATH}'/diffusion_pytorch_model-00007-of-00007.safetensors"
    ],
    "'${MODEL_BASE_PATH}'/models_t5_umt5-xxl-enc-bf16.pth",
    "'${MODEL_BASE_PATH}'/Wan2.1_VAE.pth",
    "'${MODEL_BASE_PATH}'/models_clip_open-clip-xlm-roberta-large-vit-huge-14.pth"
    ]' \
  ...
  --lora_rank ${LORA_RANK} \
  --extra_inputs "input_image","condition_rgb" \
  ...
  echo "🔧 Using standard multi-GPU training without DeepSpeed"
