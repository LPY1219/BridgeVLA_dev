...
export NCCL_DEBUG=INFO  # 减少NCCL调试输出
...
# 多机多卡配置（自动检测或手动配置）
# ==============================================
# 优先从环境变量自动获取（云服务器通常会自动设置这些变量）
# 参考 torch.distributed.launch 的参数：
#   --nnodes=${WORLD_SIZE}      -> WORLD_SIZE 环境变量
#   --node_rank=${RANK}         -> RANK 环境变量
#   --master_addr=${MASTER_ADDR} -> MASTER_ADDR 环境变量
#   --master_port=${MASTER_PORT} -> MASTER_PORT 环境变量
#   --nproc_per_node=2         -> NUM_GPUS（当前机器的GPU数）
...
if [ -n "${WORLD_SIZE}" ] && [ -n "${RANK}" ] && [ -n "${MASTER_ADDR}" ] && [ -n "${MASTER_PORT}" ]; then
    ...
    # 自动计算GPU数量：机器数量 × 8（默认每台机器8卡）
    NUM_GPUS=$((NUM_MACHINES * 8))
    echo "自动计算GPU数量: ${NUM_GPUS} (${NUM_MACHINES}台机器 × 8卡/台)"
...
#   "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_reach_expert.zarr"
#   "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_push-wall_expert.zarr"
#   "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_handle-pull_expert.zarr"
#   "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_lever-pull_expert.zarr"
#   "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_reach_expert.zarr"
#   "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_reach-wall_expert.zarr"
#   "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_peg-unplug-side_expert.zarr"
#   "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_dial-turn_expert.zarr"
  "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_door-open_expert.zarr"
  "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_door-close_expert.zarr"
#   "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_basketball_expert.zarr"
  "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_shelf-place_expert.zarr"
  "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_button-press_expert.zarr"
  "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_button-press-topdown_expert.zarr"
  "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_faucet-close_expert.zarr"
  "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_faucet-open_expert.zarr"
  "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_handle-press_expert.zarr"
  "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_hammer_expert.zarr"
#   "/mnt/data/cyx/datasets/metaworld_six_cams/metaworld_assembly_expert.zarr"
)
...
"/mnt/robot-rfm/user/lpy/data/MetaWorld/metaworld_assembly_expert.zarr"
"/mnt/robot-rfm/user/lpy/data/MetaWorld/metaworld_basketball_expert.zarr"
...
MACHINE2_MODEL_BASE_PATH="/mnt/robot-rfm/user/lpy/huggingface/Wan2.2-TI2V-5B"      # TODO: 填写当前机器的模型基础路径

# 机器3的路径配置
MACHINE3_HEATMAP_DATA_ROOT="/DATA/disk0/lpy/3D-Diffusion-Policy/3D-Diffusion-Policy/data/metaworld_door-open_expert.zarr"  # TODO: 根据实际任务修改
...
# 机器4的路径配置
MACHINE4_HEATMAP_DATA_ROOT="/DATA/disk1/lpy_a100_4/data/Franka_data_3zed_2/put_red_bull_in_pink_plate"  # TODO: 根据实际任务修改
...
TRAIL_END="4"            # 结束trail编号，如50表示到trail_50结束。留空表示不限制
...
OUTPUT_PATH="${OUTPUT_BASE}/Wan2.2-TI2V-5B_heatmap_rgb_lora/metaworld_2new_task/${TIMESTAMP}"
...
SWANLAB_PROJECT="Wan2.2-TI2V-5B_heatmap_rgb_metaworld_10_tasks"  # SwanLab项目名称
...
echo "----------------------------------------------------------------"
echo "多机多卡配置:"
if [ "${MULTI_NODE_AUTO_CONFIG}" = "true" ]; then
    echo "  配置来源: 自动从环境变量获取 (云服务器自动配置)"
    echo "  WORLD_SIZE=${WORLD_SIZE} -> 机器总数: ${NUM_MACHINES}"
    echo "  RANK=${RANK} -> 当前机器rank: ${MACHINE_RANK}"
    echo "  MASTER_ADDR=${MASTER_ADDR} -> 主节点IP: ${MAIN_PROCESS_IP}"
    echo "  MASTER_PORT=${MASTER_PORT} -> 通信端口: ${MAIN_PROCESS_PORT}"
...
if [ "${NUM_MACHINES}" -gt 1 ]; then
    ...
    accelerate launch \
        ...
        examples/wanvideo/model_training/heatmap_train_mv_view_metaworld.py \
