    ...
    **直接使用Latents**: 不使用VAE decoder的中间特征，直接使用VAE编码后的latents
    ...
    def __init__(
        ...
        self.gripper_head = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim // 2, 21)  # [-1, 1] 0.10间隔的21个bin
        )
...
def collate_fn_with_vae(batch, vae_extractor, heatmap_latent_scale=1.0, latent_noise_std=0.0, return_heatmap_images=False, num_history_frames=1):
    ...
    gripper_targets = sample['gripper_targets']  # (t-1,) - change targets
        ...
        with accelerator.accumulate(model):
            ...
            if 'img_locations' in batch:
                ...
                with torch.no_grad():
                    ...
                    # 完全解码heatmap
                    heatmap_images_flat = vae.decode(heatmap_latents_flat, device=accelerator.device)  # (b*3, 3, t_full, H, W)
