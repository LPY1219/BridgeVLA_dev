"""
RoboWan Client for Real Robot Control - Cook-3 Task
Provides both test client and real robot control loop for cook-3 task.

Cook-3 任务特点：
- 不使用夹爪
- x, y, z, rotation 都可以移动（不锁定任何轴）
- 任务指令: "Scoop the pancake and place it on the tray"
"""
...
# Cook-3 任务的初始关节配置（从 teleop_lpy_cook.py 获取）
RESET_JOINTS_COOK_3 = np.array([1.684841132467561, 0.8842280307586431, -1.7218389136359598, -1.8827389188329948, 0.8921751896732253, 1.673525678267762, 2.165940937432027])
...
def reset_joints_cook_3(robot):
    """
    将机械臂重置到 cook-3 任务的初始关节状态

    Args:
        robot: RobotController 实例
    """
    rospy.loginfo("[Cook-3 Reset] 将机械臂移动到 cook-3 初始状态...")
    rospy.loginfo(f"[Cook-3 Reset] 目标关节位置: {RESET_JOINTS_COOK_3}")
    try:
        # 先停止当前的 skill
        try:
        ...
        robot.franka.goto_joints(
            RESET_JOINTS_COOK_3.tolist(),
            duration=5.0,
            ignore_virtual_walls=True
        )
        rospy.loginfo("[Cook-3 Reset] 机械臂已移动到 cook-3 初始状态")
        ...
        rospy.loginfo(f"[Cook-3 Reset] 当前关节位置: {current_joints}")

    except Exception as e:
        rospy.logerr(f"reset_joints_cook_3 失败: {e}")
        raise
...
def real_robot_control_loop(
    server_url: str = "http://localhost:5555",
    task_prompt: str = "Scoop the pancake and place it on the tray",
    ...
    # 初始化机器人控制器
    # Cook-3任务不需要夹爪，设置 with_gripper=False
    rospy.loginfo("Initializing robot controller (without gripper for Cook-3 task)...")
    robot = RobotController(frequency=10, with_gripper=False)
    rospy.loginfo("✓ Robot controller initialized (gripper disabled)")

    # 注册退出时的清理函数，确保程序意外退出时也能清理
    atexit.register(lambda: robot.cleanup())
    rospy.loginfo("✓ Exit cleanup handler registered")

    # Cook-3 任务：先重置到初始关节位置
    rospy.loginfo("="*60)
    rospy.loginfo("[Cook-3] Resetting robot to cook-3 initial position...")
    reset_joints_cook_3(robot)
    rospy.loginfo("✓ Robot reset to cook-3 initial position")
    rospy.loginfo("="*60)

    # 记录初始位姿（Cook-3 任务中 x, y, z, rotation 都可以移动）
    initial_pose = robot.get_pose()
    rospy.loginfo(f"[Cook-3] 初始位置: {initial_pose.translation}")
    rospy.loginfo(f"[Cook-3] 初始旋转 (rad): {initial_pose.euler_angles}")
    rospy.loginfo(f"[Cook-3] 初始旋转 (deg): {np.rad2deg(initial_pose.euler_angles)}")
        ...
        while step < max_steps:
            ...
            for action_idx in range(num_actions_to_execute):
                ...
                try:
                    ...
                    # Cook-3 任务：x, y, z 和 rotation 都允许移动，不锁定任何轴
                    ...
                    # 计算目标位姿（使用绝对动作）
                    # single_action[2]=max(single_action[2]-0.03,0.05) # modify it !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    ...
                    rospy.loginfo(f"  Target position: {target_pose.translation}")
                    rospy.loginfo(f"  Target rotation: {target_pose.rotation}")
                    # 执行位姿
...
def main():
  ...
  python RoboWan_client.py --disable-action-clipping
    ...
    parser.add_argument(
        '--task',
        type=str,
        default="Scoop the pancake and place it on the tray",
        help='任务指令'
    )
