"""
Debug script for visualizing RGB and Heatmap images from dataset
Based on debug_image_input.py but reads from dataset instead of cameras

Usage:
    python debug_image_from_dataset.py --dataset_path /path/to/dataset --trail_idx 1 --frame_idx 0
    python debug_image_from_dataset.py --dataset_path /path/to/dataset --trail_idx 1 --continuous
    python debug_image_from_dataset.py --save-dir ./debug_output
"""
...
class DatasetLoader:
    """Dataset loader for loading point clouds, RGB images, and poses from dataset"""

    def __init__(self, dataset_path: str, trail_idx: int = 1):
        """
        Initialize dataset loader

        Args:
            dataset_path: Path to dataset (e.g., /path/to/put_red_bull_in_pink_plate)
            trail_idx: Trail index (1-based)
        """
        self.dataset_path = Path(dataset_path)
        self.trail_idx = trail_idx
        self.trail_path = self.dataset_path / f"trail_{trail_idx}"

        if not self.trail_path.exists():
            raise FileNotFoundError(f"Trail path not found: {self.trail_path}")

        # Load extrinsics
        extrinsics_path = self.trail_path / 'extrinsics.pkl'
        with open(extrinsics_path, 'rb') as f:
            self.extrinsics = pickle.load(f)

        # Count number of frames
        poses_dir = self.trail_path / 'poses'
        self.num_frames = len(list(poses_dir.glob('*.pkl')))

        # Load instruction
        instruction_path = self.trail_path / 'instruction.txt'
        ...
        print(f"[DatasetLoader] Loaded trail {trail_idx}")
        print(f"  - Path: {self.trail_path}")
    ...
    def load_frame(self, frame_idx: int):
        ...
        Returns:
            observation: Dictionary containing data from three cameras
                - '3rd_1', '3rd_2', '3rd_3': each with 'rgb' and 'pcd'
            pose: Robot pose array [7] - (x, y, z, qw, qx, qy, qz)
        """
        if frame_idx < 0 or frame_idx >= self.num_frames:
            raise ValueError(f"Frame index {frame_idx} out of range [0, {self.num_frames})")

        frame_name = f"{frame_idx:06d}.pkl"
        observation = {}

        # Load data from three cameras
        for cam_type in ['3rd_1', '3rd_2', '3rd_3']:
            # Load BGR image
            bgr_path = self.trail_path / f'{cam_type}_bgr' / frame_name
            with open(bgr_path, 'rb') as f:
                bgr = pickle.load(f)

            # Load point cloud
            pcd_path = self.trail_path / f'{cam_type}_pcd' / frame_name
            ...
            observation[cam_type] = {
                'rgb': rgb,
                'pcd': pcd
            }

        # Load pose
        pose_path = self.trail_path / 'poses' / frame_name
        with open(pose_path, 'rb') as f:
            pose = pickle.load(f)

        return observation, pose

    def get_extrinsic(self, cam_type: str):
        """Get extrinsic matrix for a camera"""
        return self.extrinsics[cam_type]


class ImageDebuggerFromDataset:
    """Debug class for visualizing RGB and Heatmap images from dataset"""
    ...
    def preprocess(self, pcd_list, feat_list, all_poses: np.ndarray, extrinsics: dict):
        """
        Preprocess point clouds and poses

        Args:
            pcd_list: List of point clouds [[pcd_cam1, pcd_cam2, pcd_cam3]]
            feat_list: List of features [[feat_cam1, feat_cam2, feat_cam3]]
            all_poses: Pose array [num_poses, 7] - (x,y,z,qw,qx,qy,qz)
            extrinsics: Dictionary of extrinsic matrices for each camera
        ...
        for frame_idx in range(num_frames):
            ...
            # Apply extrinsic transforms using dataset extrinsics
            pcd_cam1_base = self.convert_pcd_to_base(extrinsic_martix=extrinsics['3rd_1'], pcd=frame_pcds[0])
            pcd_cam2_base = self.convert_pcd_to_base(extrinsic_martix=extrinsics['3rd_2'], pcd=frame_pcds[1])
            pcd_cam3_base = self.convert_pcd_to_base(extrinsic_martix=extrinsics['3rd_3'], pcd=frame_pcds[2])
...
def process_and_debug(debugger, dataset_loader, frame_idx, save_dir=None):
    """
    Process a frame from dataset and debug visualization

    Args:
        debugger: ImageDebuggerFromDataset instance
        dataset_loader: DatasetLoader instance
        frame_idx: Frame index to process
        save_dir: Optional directory to save images

    Returns:
        rgb_images, heatmap_images, processed_pcd, processed_rgb
    """
    ...
    observation, pose = dataset_loader.load_frame(frame_idx)

    # Get RGB and point clouds
    rgb_3rd_1 = observation['3rd_1']['rgb']
    rgb_3rd_2 = observation['3rd_2']['rgb']
    rgb_3rd_3 = observation['3rd_3']['rgb']

    pcd_3rd_1 = observation['3rd_1']['pcd']
    pcd_3rd_2 = observation['3rd_2']['pcd']
    pcd_3rd_3 = observation['3rd_3']['pcd']

    # Parse pose: [x, y, z, qw, qx, qy, qz]
    current_position = pose[:3]
    current_quat = pose[3:]  # [qw, qx, qy, qz]

    # Convert to scipy format [x, y, z, w] for rotation display
    ...
    print(f"Current quaternion (wxyz): {current_quat}")
    print(f"Current rotation (deg): {current_rotation_deg}")

    # Create pose array
    current_pose = pose.reshape(1, 7)

    # Get extrinsics from dataset
    extrinsics = {
        '3rd_1': dataset_loader.get_extrinsic('3rd_1'),
        '3rd_2': dataset_loader.get_extrinsic('3rd_2'),
        '3rd_3': dataset_loader.get_extrinsic('3rd_3')
    }
    ...
    if save_dir:
        ...
        save_path = save_dir / f"debug_frame{frame_idx:06d}_{timestamp}.png"

        # Also save individual images
        for i, img in enumerate(rgb_images):
            img.save(save_dir / f"rgb_view{i+1}_frame{frame_idx:06d}_{timestamp}.png")
        for i, img in enumerate(heatmap_images):
            img.save(save_dir / f"heatmap_view{i+1}_frame{frame_idx:06d}_{timestamp}.png")
        for i, img in enumerate(raw_rgb_images):
            img.save(save_dir / f"raw_cam{i+1}_frame{frame_idx:06d}_{timestamp}.png")

    # Visualize
    debugger.visualize_all(
        rgb_images, heatmap_images, raw_rgb_images,
        peak_positions=peak_positions,
        title=f"Frame {frame_idx} - Trail {dataset_loader.trail_idx}",
        save_path=save_path
    )
...
def main():
    parser = argparse.ArgumentParser(description="Debug RGB and Heatmap images from dataset")

    parser.add_argument('--dataset_path', type=str,
                        default='/media/casia/data4/lpy/3zed_data/filter_data_5/cook_5',
                        help='Path to dataset')
    parser.add_argument('--trail_idx', type=int, default=5,
                        help='Trail index (1-based)')
    parser.add_argument('--frame_idx', type=int, default=32,
                        help='Frame index to process')
    parser.add_argument('--continuous', action='store_true',
                        help='Continuous mode (navigate through frames)')
    ...
    # Initialize
    print("="*60)
    print("Image Input Debugger (Dataset Mode)")
    print("="*60)

    # Initialize dataset loader
    try:
        dataset_loader = DatasetLoader(args.dataset_path, args.trail_idx)
    except FileNotFoundError as e:
        print(f"Error: {e}")
        return

    # Initialize debugger
    debugger = ImageDebuggerFromDataset(
    ...
    try:
        if args.continuous:
            ...
            print(f"Total frames: {dataset_loader.num_frames}")
            print("Commands: n=next, p=prev, [number]=goto frame, v=visualize pcd, q=quit")
            print("="*60)

            current_frame = args.frame_idx

            while True:
                # Process current frame
                rgb_images, heatmap_images, pcd, rgb, peak_pos = process_and_debug(
                    debugger, dataset_loader, current_frame, args.save_dir
                )

                # Get user input
                user_input = input(f"\nFrame {current_frame}/{dataset_loader.num_frames-1} > ").strip().lower()

                if user_input == 'q':
                    break
                elif user_input == 'n':
                    current_frame = min(current_frame + 1, dataset_loader.num_frames - 1)
                elif user_input == 'p':
                    current_frame = max(current_frame - 1, 0)
                elif user_input == 'v':
                    debugger.visualize_pointcloud(pcd, rgb, f"Point Cloud Frame {current_frame}")
                elif user_input.isdigit():
                    new_frame = int(user_input)
                    if 0 <= new_frame < dataset_loader.num_frames:
                        current_frame = new_frame
                    else:
                        print(f"Invalid frame index. Valid range: [0, {dataset_loader.num_frames-1}]")
                elif user_input == '':
                    # Default: next frame
                    current_frame = min(current_frame + 1, dataset_loader.num_frames - 1)
        else:
            # Single frame
            rgb_images, heatmap_images, pcd, rgb, peak_pos = process_and_debug(
                debugger, dataset_loader, args.frame_idx, args.save_dir
            )

            if args.visualize_pcd:
                debugger.visualize_pointcloud(pcd, rgb, f"Point Cloud Frame {args.frame_idx}")
