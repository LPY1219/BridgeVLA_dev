...
def real_robot_control_loop(
    ...
    # 使用阻塞式控制模式（每个action等待到达）
    rospy.loginfo("Using BLOCKING control mode (每个action会等待到达目标)")
    ...
    rospy.loginfo("✓ Ready for blocking control (不使用dynamic模式)")
    ...
    rospy.loginfo("="*60)
    rospy.loginfo("✓ Rotation and Gripper will be controlled by MODEL prediction")
    rospy.loginfo("="*60 + "\n")
    ...
    try:
        while step < max_steps:
            ...
            try:
                ...
                rospy.loginfo(f"Current rotation (used for inference): {current_rotation_deg}")
            ...
            rospy.loginfo(f"✓ Using MODEL predicted rotation actions (converted from degrees to radians)")
            rospy.loginfo(f"✓ Using MODEL predicted gripper actions")
            ...
            # ========== 6. 执行动作（阻塞式控制）==========
            # 策略：每个action使用阻塞式goto_pose，等待到达后再执行下一个
            # 优点：精确到达每个目标，无旧目标干扰
            # 缺点：控制频率较低，动作可能不够连贯
            rospy.loginfo("Preparing to execute action chunk (blocking mode)...")
            ...
            for action_idx in range(num_actions_to_execute):
                ...
                rospy.loginfo(f"  Rotation: {np.rad2deg(single_action[3:6])} (degrees)")
                ...
                try:
                    ...
                    # ⚠️ 阻塞式执行：等待机器人到达目标位置后再继续
                    # use_impedance=False: 使用Franka原生控制器（更精确）
                    # dynamic=False: 不使用dynamic模式
                    # block=True: 阻塞模式，等待到达目标后才返回
                    t_start = time.time()
                    robot.franka.goto_pose(
                        target_pose,
                        duration=action_duration,
                        use_impedance=False,  # 使用Franka原生控制器，不是阻抗控制
                        dynamic=False,         # 不使用dynamic模式
                        block=True,            # ✓ 阻塞模式，等待到达
                        ignore_errors=True,
                    ...
                    rospy.loginfo(f"  ✓ Action {action_idx+1} completed in {elapsed:.2f}s (final distance: {final_distance*100:.2f}cm)")
