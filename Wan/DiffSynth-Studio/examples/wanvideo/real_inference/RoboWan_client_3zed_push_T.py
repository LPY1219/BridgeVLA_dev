...
# Push-T 任务的初始关节配置
RESET_JOINTS_PUSH_T = np.array([-0.31877957, 0.17202022, -0.59794134, -2.3979113, 0.18720852, 2.52827357, -0.27203674])
...
def reset_joints_push_t(robot):
    """
    将机械臂重置到 push-T 任务的初始关节状态

    Args:
        robot: RobotController 实例
    """
    rospy.loginfo("[Push-T Reset] 将机械臂移动到 push-T 初始状态...")
    rospy.loginfo(f"[Push-T Reset] 目标关节位置: {RESET_JOINTS_PUSH_T}")
    try:
        # 先停止当前的 skill
        try:
        ...
        robot.franka.goto_joints(
            RESET_JOINTS_PUSH_T.tolist(),
            duration=5.0,
            ignore_virtual_walls=True
        )
        rospy.loginfo("[Push-T Reset] 机械臂已移动到 push-T 初始状态")
        ...
        rospy.loginfo(f"[Push-T Reset] 当前关节位置: {current_joints}")
...
def real_robot_control_loop(
    server_url: str = "http://localhost:5555",
    task_prompt: str = "push the T block into the target",
    ...
    # 初始化机器人控制器
    # Push-T任务不需要夹爪，设置 with_gripper=False
    rospy.loginfo("Initializing robot controller (without gripper for Push-T task)...")
    ...
    # Push-T 任务：先重置到初始关节位置
    rospy.loginfo("="*60)
    rospy.loginfo("[Push-T] Resetting robot to push-T initial position...")
    reset_joints_push_t(robot)
    rospy.loginfo("✓ Robot reset to push-T initial position")
    rospy.loginfo("="*60)

    # 记录初始 z 轴高度和旋转（Push-T 任务中保持不变）
    initial_pose = robot.get_pose()
    initial_z = initial_pose.translation[2]
    initial_rotation = initial_pose.euler_angles  # [roll, pitch, yaw] in radians
    rospy.loginfo(f"[Push-T] 初始 z 轴高度: {initial_z:.4f}m (已锁定)")
    rospy.loginfo(f"[Push-T] 初始旋转 (rad): {initial_rotation} (已锁定)")
    rospy.loginfo(f"[Push-T] 初始旋转 (deg): {np.rad2deg(initial_rotation)} (已锁定)")
        ...
        while step < max_steps:
            ...
            for action_idx in range(num_actions_to_execute):
                ...
                try:
                    ...
                    # Push-T 任务：强制锁定 z 轴和旋转值
                    # 忽略预测的 z 值和旋转值，使用初始值
                    original_z = single_action[2]
                    original_rotation = single_action[3:6].copy()
                    single_action[2] = initial_z  # 锁定 z 轴
                    single_action[3:6] = initial_rotation  # 锁定旋转 (已经是弧度)
                    rospy.loginfo(f"  [Push-T] Z locked: {original_z:.4f} -> {initial_z:.4f}")
                    rospy.loginfo(f"  [Push-T] Rotation locked: {np.rad2deg(original_rotation)} -> {np.rad2deg(initial_rotation)}")
...
def main():
  ...
  python RoboWan_client.py --disable-action-clipping
    ...
    parser.add_argument(
        '--task',
        type=str,
        default="push the T block into the target",
        help='任务指令'
    )
