"""
Inference Script for Multi-View View-Concatenation Model

This script performs inference using WanModel with view-concatenation mode,
where different views are concatenated along the batch dimension instead of
using token concatenation.

Key differences from mv_concat:
- Uses standard WanModel (no special conversion needed)
- Views are concatenated in batch dimension
- Uses wan_video_5B_TI2V_heatmap_and_rgb_mv_view pipeline
- Supports use_merged_pointcloud and use_different_projection

参考自: heatmap_inference_mv_concat.py
"""
...
class HeatmapInferenceMVView:
    """
    Inference class for multi-view view-concatenation model.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            wan_type: Model type (default: 5B_TI2V_RGB_HEATMAP_MV)
        ...
        print("="*60)
        print("Initializing MV View Inference")
        print("="*60)
    ...
    def _load_pipeline(self, model_base_path: str, lora_checkpoint: str):
        ...
        # No model conversion needed for view-concatenation mode
        print("\nStep 2: Model loaded successfully (view-concat mode, no conversion needed)")

        # Add multi-view attention modules (same as training script)
    ...
    def _load_checkpoint_weights(self, pipe, checkpoint_path: str):
        """
        加载checkpoint中的非LoRA权重（head, patch_embedding, mvs_attn等）
        ...
        try:
            ...
            if mvs_weights:
                print("  Multi-view module keys (sample):")
                for key in list(mvs_weights.keys())[:5]:
                    print(f"    - {key}")
            ...
            if missing_keys:
                relevant_missing = [k for k in missing_keys if any(p in k for p in
                    ['head', 'patch_embedding', 'modulation', 'mvs_attn', 'projector'])]
                if relevant_missing:
    ...
    def _load_lora_with_base_weights(self, pipe, checkpoint_path: str, alpha: float = 1.0):
        ...
        # Step 1: 先加载所有非LoRA权重
        print("\n  Step 3.1: Loading non-LoRA weights (head, patch_embedding, mvs modules, etc.)...")
        ...
        if hasattr(pipe.dit, 'patch_embedding') and pipe.dit.patch_embedding is not None:
            ...
            print(f"\n    [DEBUG] patch_embedding is None or does not exist")
        ...
        try:
            ...
            if missing_keys:
                important_missing = [k for k in missing_keys if any(p in k for p in
                    ['patch_embedding', 'head', 'self_attn', 'cross_attn', 'ffn', 'mvs_attn'])]
    ...
    def run_inference(
        ...
        NOTE: View-concatenation mode concatenates views in batch dimension,
        which is more efficient than token concatenation.
        ...
        print(f"  NOTE: View-concat mode (views in batch dimension)")
...
def test_on_dataset_mv_view(
    inference_engine: HeatmapInferenceMVView,
    data_root: str,
    output_dir: str,
    ...
    Args:
        ...
        image_size: 图像尺寸
        scene_bounds: 场景边界
        transform_augmentation_xyz: xyz变换增强
        transform_augmentation_rpy: rpy变换增强
        wan_type: WAN模型类型
        use_merged_pointcloud: 是否使用融合点云
        use_different_projection: 是否使用不同投影模式
        trail_start: 起始轨迹编号
        trail_end: 结束轨迹编号
    """
    ...
    if trail_start is not None:
        print(f"Trail start: {trail_start}")
    if trail_end is not None:
        print(f"Trail end: {trail_end}")
    ...
    if all_heatmap_distances:
        ...
        with open(stats_path, 'w') as f:
            f.write("MV View Evaluation Results\n")
            f.write("="*60 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(description="MV View Inference")
    ...
    parser.add_argument("--trail_start", type=int, default=None,
                       help='Starting trail number (e.g., 1 for trail_1)')
    parser.add_argument("--trail_end", type=int, default=None,
                       help='Ending trail number (e.g., 5 for trail_5)')
    ...
    print(f"Use Merged Pointcloud: {args.use_merged_pointcloud}")
    print(f"Use Different Projection: {args.use_different_projection}")
    print(f"Data Root: {args.data_root}")
    print(f"Output Dir: {args.output_dir}")
    print()

    # 创建推理器
    inference = HeatmapInferenceMVView(
    ...
    if args.data_root is None:
        ...
        num_views = 3  # Default 3 views for RGB cameras
