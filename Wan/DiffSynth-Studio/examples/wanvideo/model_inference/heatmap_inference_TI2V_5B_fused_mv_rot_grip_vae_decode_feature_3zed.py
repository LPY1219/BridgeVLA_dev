...
# 根据 wan_type 在 __init__ 中动态导入正确的 pipeline
# 单帧模式: wan_video_5B_TI2V_heatmap_and_rgb_mv_rot_grip
# 多帧历史模式: wan_video_5B_TI2V_heatmap_and_rgb_mv_history
...
class HeatmapInferenceMVRotGrip:
    ...
    def __init__(self,
        ...
        Args:
            ...
            is_full_finetune: 是否为全量微调的checkpoint（非LoRA）
        """
        self.device = device
        ...
        print(f"  num_history_frames: {num_history_frames}")
        ...
        if num_history_frames > 1 and wan_type != "5B_TI2V_RGB_HEATMAP_MV_ROT_GRIP_HISTORY":
            raise ValueError(
                ...
                f"  Please either:\n"
                f"    - Set wan_type='5B_TI2V_RGB_HEATMAP_MV_ROT_GRIP_HISTORY' for multi-frame history mode\n"
                f"    - Or set num_history_frames=1 for single-frame mode"
            )
        if num_history_frames == 1 and wan_type == "5B_TI2V_RGB_HEATMAP_MV_ROT_GRIP_HISTORY":
            print(f"  [WARNING] wan_type is HISTORY mode but num_history_frames=1, will use single-frame mode")

        # 根据 wan_type 动态导入正确的 pipeline
        if wan_type == "5B_TI2V_RGB_HEATMAP_MV_ROT_GRIP":
            from diffsynth.pipelines.wan_video_5B_TI2V_heatmap_and_rgb_mv_rot_grip import WanVideoPipeline
            print("  Loading single-frame pipeline: wan_video_5B_TI2V_heatmap_and_rgb_mv_rot_grip")
        elif wan_type == "5B_TI2V_RGB_HEATMAP_MV_ROT_GRIP_HISTORY":
            from diffsynth.pipelines.wan_video_5B_TI2V_heatmap_and_rgb_mv_history import WanVideoPipeline
            print("  Loading multi-frame history pipeline: wan_video_5B_TI2V_heatmap_and_rgb_mv_history")
        else:
            raise ValueError(f"Unsupported wan_type: {wan_type}. Use '5B_TI2V_RGB_HEATMAP_MV_ROT_GRIP' or '5B_TI2V_RGB_HEATMAP_MV_ROT_GRIP_HISTORY'")
        ...
        # 加载diffusion模型权重（根据训练方式选择不同的加载策略）
        if self.is_full_finetune:
            # 全量微调：加载所有训练过的权重
            print(f"Loading full finetune checkpoint: {lora_checkpoint_path}")
            self.load_full_finetune_checkpoint(lora_checkpoint_path)
        else:
            # LoRA训练：先加载base layer，再应用LoRA变化量
        ...
        print(f"  use_initial_gripper_state: {use_initial_gripper_state}")
        self.rot_grip_predictor = MultiViewRotationGripperPredictor(
            ...
            dropout=0.1,
            local_feature_size=local_feat_size,  # 局部特征提取的邻域大小
            use_initial_gripper_state=use_initial_gripper_state,  # 是否使用初始夹爪状态作为输入
    ...
    def _decode_latents_with_history(self, latents, return_full_image=False):
        """
        根据 num_history_frames 解码 latents 为中间特征

        当 num_history_frames=2 时，latents 的结构是：
        - latent[:,:,0:1] - 第1帧单独编码
        - latent[:,:,1:] - 第2帧+剩余帧正常编码

        需要分别解码并 concat，确保输出帧数与输入帧数一致。

        Args:
            latents: (c, t_compressed, h, w) - 单个视角的 latent
            return_full_image: 是否同时返回完全解码的图像

        Returns:
            如果 return_full_image=False:
                intermediate_features: (c_intermediate, t_upsampled, h, w)
            如果 return_full_image=True:
                (intermediate_features, full_images)
        """
        if self.num_history_frames == 2 and latents.shape[1] >= 2:
            # 特殊处理：分别解码第1帧和剩余帧
            # 第1帧的 latent
            latent_0 = latents[:, 0:1, :, :]  # (c, 1, h, w)

            # 剩余帧的 latent
            latent_remaining = latents[:, 1:, :, :]  # (c, t-1, h, w)

            if return_full_image:
                # 解码第1帧
                feat_0, full_0 = self.vae_decode_intermediate.decode_intermediate_with_full(
                    [latent_0], device=self.device
                )
                feat_0 = feat_0[0]  # (c_intermediate, t=1, h, w)
                full_0 = full_0[0]  # (3, t=1, H, W)

                # 解码剩余帧
                feat_remaining, full_remaining = self.vae_decode_intermediate.decode_intermediate_with_full(
                    [latent_remaining], device=self.device
                )
                feat_remaining = feat_remaining[0]  # (c_intermediate, t_remaining, h, w)
                full_remaining = full_remaining[0]  # (3, t_remaining, H, W)

                # Concat
                intermediate_features = torch.cat([feat_0, feat_remaining], dim=1)
                full_images = torch.cat([full_0, full_remaining], dim=1)
                return intermediate_features, full_images
            else:
                # 解码第1帧
                feat_0 = self.vae_decode_intermediate.decode_intermediate(
                    [latent_0], device=self.device
                )
                feat_0 = feat_0[0]

                # 解码剩余帧
                feat_remaining = self.vae_decode_intermediate.decode_intermediate(
                    [latent_remaining], device=self.device
                )
                feat_remaining = feat_remaining[0]
            ...
            # 正常解码
            if return_full_image:
                intermediate_features, full_images = self.vae_decode_intermediate.decode_intermediate_with_full(
                    [latents], device=self.device
                )
                return intermediate_features[0], full_images[0]
            else:
                intermediate_features = self.vae_decode_intermediate.decode_intermediate(
                    [latents], device=self.device
                )
                return intermediate_features[0]

    def _encode_video_with_history(self, video_frames, tiled=False, tile_size=(34, 34), tile_stride=(18, 16)):
        """
        根据 num_history_frames 编码单个视角的视频

        当 num_history_frames=2 时，需要分别编码:
        - 第1帧单独编码 → 1个 latent
        - 第2帧+剩余帧正常编码 → 若干个 latent
        然后 concat 这两部分，确保解码时能正确还原帧数

        Args:
            video_frames: List[PIL.Image] - 单个视角的视频帧列表 (T帧)
            tiled: 是否使用 tiled 编码
            tile_size: tile 大小
            tile_stride: tile 步长

        Returns:
            latents: (c, t_compressed, h, w) - 编码后的 latent
            其中 t_compressed 的值:
            - num_history_frames=1: (T-1)//4+1
            - num_history_frames=2: 1 + ((T-1)-1)//4+1 = 1 + (T-2)//4+1
        """
        vae = self.pipe.vae
        num_frames = len(video_frames)

        if self.num_history_frames == 2 and num_frames >= 2:
            # 特殊处理：第1帧单独编码，第2帧+剩余帧作为新视频编码

            # Frame 0 单独编码
            frame_0_video = self.preprocess_video([video_frames[0]])
            frame_0_video = frame_0_video.squeeze(0)  # (C, T=1, H, W)
            z_0 = vae.encode(
                [frame_0_video],
                device=self.device,
                tiled=tiled,
            ...
            z_0 = z_0[0].to(dtype=self.torch_dtype, device=self.device)  # (c, t=1, h, w)

            # Frame 1 + 剩余帧作为新视频编码
            remaining_frames = video_frames[1:]  # 从第2帧开始（共 num_frames-1 帧）
            remaining_video = self.preprocess_video(remaining_frames)
            remaining_video = remaining_video.squeeze(0)  # (C, T=num_frames-1, H, W)
            z_remaining = vae.encode(
                [remaining_video],
                device=self.device,
                tiled=tiled,
                tile_size=tile_size,
            ...
            z_remaining = z_remaining[0].to(dtype=self.torch_dtype, device=self.device)  # (c, t', h, w)

            # 在时间维度上 concat: [第1帧 latent, 剩余帧 latent]
            latents = torch.cat([z_0, z_remaining], dim=1)  # (c, 1+t', h, w)

        else:
            # 正常编码（num_history_frames=1 或 1+4N）
    ...
    def load_checkpoint_weights(self, checkpoint_path: str):
        ...
        关键点：
        1. checkpoint中的多视角注意力权重以 `base_layer` 命名（如 blocks.0.mvs_attn.k.base_layer.weight），
           但模型中的参数名没有 `base_layer`（如 blocks.0.mvs_attn.k.weight），需要转换。
        2. 训练时可能会训练 modulation (AdaLN调制参数) 和 mvs_attn.norm_q/norm_k 参数，
           这些参数也会保存在checkpoint中，需要一并加载。
        ...
        try:
            ...
            mv_other_weights = {}  # 其他 MV 相关权重（projector, norm_mvs, modulation_mvs, mvs_attn中的norm等）
            modulation_weights = {}  # AdaLN 调制参数（modulation）
            ...
            for key, value in state_dict.items():
                ...
                # 筛选 modulation 参数（AdaLN调制参数）
                # 注意：这里单独匹配 'modulation'（不带mvs后缀），因为训练时可能会解冻这些参数
                # 需要区分 modulation 和 modulation_mvs
                elif key.endswith('.modulation') or '.modulation' in key and 'mvs' not in key:
                    # 匹配 blocks.X.modulation 格式的key
            ...
            print(f"  Found {len(modulation_weights)} modulation (AdaLN) weights")
            ...
            if modulation_weights:
                print("  Modulation (AdaLN) keys (sample):")
                for key in list(modulation_weights.keys())[:5]:
                    print(f"    - {key}")
            ...
            weights_to_load.update(modulation_weights)  # 添加 modulation (AdaLN) 权重
            ...
            if missing_keys:
                relevant_missing = [k for k in missing_keys if any(p in k for p in ['patch_embedding', 'head', 'projector', 'mvs_', 'modulation'])]
                if relevant_missing:
    ...
    def load_full_finetune_checkpoint(self, checkpoint_path: str):
        """
        加载全量微调的checkpoint（加载所有dit权重，不是LoRA）

        与LoRA checkpoint的区别：
        - 全量微调checkpoint包含所有训练过的权重（patch_embedding, head, blocks.*等）
        - 权重名称是直接的形式（如 blocks.0.mvs_attn.k.weight），没有 .base_layer 后缀
        - 不包含 lora_A, lora_B 等LoRA特定的权重

        加载策略：
        - 加载checkpoint中所有的dit相关权重（不进行特定模块筛选）
        - 确保所有训练过的模块都能被正确加载

        Args:
            checkpoint_path: 全量微调checkpoint文件路径
        """
        try:
            ...
            # 加载checkpoint
            print("Step 1: Loading state dict from checkpoint...")
            state_dict = load_state_dict(checkpoint_path)
            print(f"  ✓ Loaded {len(state_dict)} keys from checkpoint")

            # 筛选dit相关权重（排除lora相关）
            print("\nStep 2: Filtering DIT weights (excluding LoRA keys)...")
            dit_weights = {}
            lora_keys_count = 0
            for key, value in state_dict.items():
                # 跳过lora相关权重
            ...
            # 显示包含的模块类型
            module_types = set()
            for key in dit_weights.keys():
                if 'patch_embedding' in key:
                    module_types.add('patch_embedding')
                elif 'head' in key:
                    module_types.add('head')
                elif 'blocks' in key:
                    # 提取blocks中的模块名
                    parts = key.split('.')
                    if len(parts) >= 3:
                        module_types.add(f'blocks.{parts[2]}')
                elif 'text_embedding' in key:
                    module_types.add('text_embedding')
                elif 'time_embedding' in key:
                    module_types.add('time_embedding')

            print(f"  Modules included: {sorted(module_types)}")

            # 清理权重key（移除前缀）
            print("\nStep 3: Cleaning weight keys (removing prefixes)...")
            weights_clean = {}
            ...
            print(f"  ✓ Prepared {len(weights_clean)} weights for loading")

            # 加载到DIT模型中
            print("\nStep 4: Loading weights into DIT model...")
            missing_keys, unexpected_keys = self.pipe.dit.load_state_dict(
                weights_clean, strict=True
            )
            ...
            if missing_keys:
                ...
                important_missing = [k for k in missing_keys if any(p in k for p in
                    ['patch_embedding', 'head', 'mvs_attn', 'self_attn', 'cross_attn', 'ffn'])]
                if important_missing:
                    print(f"  Warning: {len(important_missing)} important keys not found in checkpoint:")
            ...
            print(f"\n✗ Error: Failed to load full finetune checkpoint: {e}")
            import traceback
            traceback.print_exc()
            raise
    ...
    def _visualize_input_images_history(self,
                                        input_images: List[List[Image.Image]],
                                        input_images_rgb: List[List[Image.Image]],
                                        prompt: str):
        """
        可视化多帧历史的多视角图像，将所有历史帧拼接成一张大图

        Args:
            input_images: List[List[PIL.Image]] - 多帧历史热力图 (num_history, num_views)
            input_images_rgb: List[List[PIL.Image]] - 多帧历史RGB图像 (num_history, num_views)
            prompt: 文本提示
        """
        import matplotlib.pyplot as plt
        import os
        ...
        # 为每个历史帧的每个视角查找峰值位置
        print("Finding peaks in input heatmaps for all history frames...")
        all_input_peaks = []  # (num_history, num_views)
        for hist_idx in range(num_history):
            hist_peaks = []
            for view_idx in range(num_views):
                peak = self.find_peak_position(input_images[hist_idx][view_idx], colormap_name='jet')
                hist_peaks.append(peak)
            all_input_peaks.append(hist_peaks)

        # 在RGB图像上标记峰值点
        print("Marking peaks on input RGB images for all history frames...")
        all_input_rgb_marked = []  # (num_history, num_views)
        for hist_idx in range(num_history):
            hist_rgb_marked = []
            for view_idx in range(num_views):
                img = input_images_rgb[hist_idx][view_idx].copy()
                draw = ImageDraw.Draw(img)
                peak_x, peak_y = all_input_peaks[hist_idx][view_idx]
                ...
                hist_rgb_marked.append(img)
            all_input_rgb_marked.append(hist_rgb_marked)

        # 创建图像网格：2*num_history行（每个历史帧有heatmap和rgb两行）× num_views列
        # 布局：第0行=历史帧1热力图, 第1行=历史帧1 RGB, 第2行=历史帧2热力图, 第3行=历史帧2 RGB, ...
        fig, axes = plt.subplots(2 * num_history, num_views,
                                figsize=(num_views * 4, 2 * num_history * 4))
        ...
        for hist_idx in range(num_history):
            # 热力图行
            heatmap_row = 2 * hist_idx
            for view_idx in range(num_views):
                ax = axes[heatmap_row, view_idx]
                ax.imshow(input_images[hist_idx][view_idx])
                ax.set_title(f"Frame {hist_idx+1} - Heatmap View {view_idx}",
                           fontsize=12, fontweight='bold')
                ax.axis('off')

            # RGB行
            rgb_row = 2 * hist_idx + 1
            for view_idx in range(num_views):
                ax = axes[rgb_row, view_idx]
                ax.imshow(all_input_rgb_marked[hist_idx][view_idx])
                ax.set_title(f"Frame {hist_idx+1} - RGB View {view_idx} (Peak Marked)",
                           fontsize=12, fontweight='bold')
                ax.axis('off')

        # 添加总标题
        fig.suptitle(f"Input History Frames (Multi-View, {num_history} frames)\nPrompt: {prompt}",
                     fontsize=14, fontweight='bold', y=0.995)

        plt.tight_layout()

        # 保存图像（使用计数器命名，避免覆盖）
        save_path = os.path.join(self._debug_img_dir, f"debug_input_history_{self._debug_img_counter:04d}.png")
        plt.savefig(save_path, dpi=150, bbox_inches='tight')
        plt.close()

        print(f"\n{'='*80}")
        print(f"INPUT HISTORY VISUALIZATION SAVED")
        print(f"{'='*80}")
        print(f"  Location: {save_path}")
        print(f"  Counter: {self._debug_img_counter}")
        print(f"  Prompt: {prompt}")
        print(f"  Num History Frames: {num_history}")
        print(f"  Num Views per Frame: {num_views}")
        print(f"  Heatmap size: {input_images[0][0].size}")
        print(f"  RGB size: {input_images_rgb[0][0].size}")
        print(f"{'='*80}\n")
    ...
    def predict(self,
                prompt: str,
                input_image: List[Image.Image] = None,  # 单帧模式：多视角起始图像 List[PIL.Image] (num_views,)
                input_image_rgb: List[Image.Image] = None,  # 单帧模式：多视角起始RGB图像
                input_images: List[List[Image.Image]] = None,  # 多帧历史模式：List[List[PIL.Image]] (num_history, num_views)
                input_images_rgb: List[List[Image.Image]] = None,  # 多帧历史模式：List[List[PIL.Image]] (num_history, num_views)
                initial_rotation: np.ndarray = None,  # (3,) - [roll, pitch, yaw] in degrees
                initial_gripper: int = None,  # 0 or 1
                num_frames: int = 5,
                height: int = 256,
                width: int = 256,
        ...
        Args:
            prompt: 文本提示
            input_image: [单帧模式] 多视角热力图起始图像 [view0, view1, view2]
            input_image_rgb: [单帧模式] 多视角RGB起始图像 [view0, view1, view2]
            input_images: [多帧历史模式] 历史热力图图像 [[t0_v0, t0_v1, t0_v2], [t1_v0, t1_v1, t1_v2], ...]
            input_images_rgb: [多帧历史模式] 历史RGB图像 [[t0_v0, t0_v1, t0_v2], [t1_v0, t1_v1, t1_v2], ...]
        ...
        # 0. 可视化输入图像（多视角）
        # 如果是多帧历史模式，将所有历史帧拼接成一张大图
        if input_images is not None:
            # 多帧历史模式：将所有历史帧拼接成一张大图
            print(f"Visualizing {len(input_images)} history frames as a grid...")
            self._visualize_input_images_history(input_images, input_images_rgb, prompt)
        else:
            # 单帧模式：只可视化一帧
        ...
        # 对于 num_history_frames=2，需要分别解码第1帧和剩余帧
        ...
        for view_idx in range(num_views):
            view_rgb_latent = rgb_latents[view_idx]  # (c, t, h, w)
            view_heatmap_latent = heatmap_latents[view_idx]  # (c, t, h, w)

            # RGB: 只需要中间特征（使用支持 history frames 的解码方法）
            rgb_intermediate = self._decode_latents_with_history(
                view_rgb_latent, return_full_image=False
            )
            all_rgb_intermediate.append(rgb_intermediate)

            # Heatmap: 需要中间特征和完全解码的图像（用于找峰值）
            heatmap_intermediate, heatmap_full = self._decode_latents_with_history(
                view_heatmap_latent, return_full_image=True
            )
            all_heatmap_intermediate.append(heatmap_intermediate)
            all_heatmap_images.append(heatmap_full)
        ...
        # 计算未来帧数量：总帧数 - 历史帧数
        # 在多帧历史模式下，num_frames 包含历史帧，所以需要减去历史帧数量
        num_future_frames = num_frames - self.num_history_frames

        # 准备history_gripper_states参数（当use_initial_gripper_state=True时需要）
        history_gripper_states = None
        if self.use_initial_gripper_state and initial_gripper is not None:
            # 创建history_gripper_states tensor: (batch=1, num_history_frames)
            # 假设所有历史帧的夹爪状态相同（使用initial_gripper填充）
        ...
        rotation_logits, gripper_logits = self.rot_grip_predictor(
            ...
            colormap_name='jet',
            num_history_frames=self.num_history_frames,  # 传递历史帧数量
            history_gripper_states=history_gripper_states,  # 传递历史帧夹爪状态（当use_initial_gripper_state=True时使用）
        )
        ...
        # 6. 可视化生成的视频帧
        if visualize:
            # 转置视频帧格式: (num_views, T) -> (T, num_views)
            num_views_output = len(video_heatmap_frames)
            num_frames_output = len(video_heatmap_frames[0])
            video_heatmap_frames_T = [[video_heatmap_frames[v][t] for v in range(num_views_output)]
                                       for t in range(num_frames_output)]
            video_rgb_frames_T = [[video_rgb_frames[v][t] for v in range(num_views_output)]
                                   for t in range(num_frames_output)]

            # 查找热力图峰值位置
            print("Finding heatmap peaks for visualization...")
            peaks = self.find_peaks_batch(video_heatmap_frames_T, colormap_name='jet')  # (T, num_views)
    ...
    def mark_peaks_on_images(self,
        ...
        Args:
            rgb_images: List[List[PIL.Image]] (T, num_views) - RGB图像
            peaks: List[List[Tuple[int, int]]] (T, num_views) - 峰值位置 (x, y)
            marker_size: 标记大小（半径）
            marker_color: 标记颜色
            marker_width: 标记边框宽度

        Returns:
            marked_images: List[List[PIL.Image]] (T, num_views) - 标记后的RGB图像
        """
        marked_images = []
    ...
    def encode_gt_videos(self, rgb_videos, heatmap_videos, tiled=False, tile_size=(34, 34), tile_stride=(18, 16)):
        ...
        当 num_history_frames=2 时，使用 _encode_video_with_history 进行特殊编码:
        - 第1帧单独编码
        - 第2帧+剩余帧正常编码
        然后 concat，确保解码时帧数正确
        ...
        for view_idx in range(num_views):
            ...
            # 使用 _encode_video_with_history 编码（自动处理 num_history_frames=2 的情况）
        ...
        for view_idx in range(num_views):
            ...
            # 使用 _encode_video_with_history 编码（自动处理 num_history_frames=2 的情况）
            view_heatmap_latents = self._encode_video_with_history(
                view_heatmap_frames,
                tiled=tiled,
                tile_size=tile_size,
                tile_stride=tile_stride
...
def visualize_heatmaps_with_peaks(colormap_images: List[List[Image.Image]],
    ...
    Args:
        ...
        save_dir: 保存可视化结果的目录 (如果为None,使用项目根目录下的默认路径)
    """
    # 设置默认的保存目录
    if save_dir is None:
        save_dir = os.path.join(ROOT_PATH, "Wan/DiffSynth-Studio/examples/wanvideo/model_inference/heatmap_inference_results/debug_img")
    import os
    from pathlib import Path
...
def test_on_dataset_mv_rot_grip(inference_engine: HeatmapInferenceMVRotGrip,
    ...
    Args:
        ...
        image_size: 图像尺寸
    """
    # Import the correct dataset module based on wan_type
    if "HISTORY" in wan_type:
        print(f"Using history dataset factory for wan_type: {wan_type}")
        from diffsynth.trainers.heatmap_dataset_mv_with_rot_grip_3cam_history import HeatmapDatasetFactoryWithHistory as HeatmapDatasetFactory
    else:
        print(f"Using standard dataset factory for wan_type: {wan_type}")
    ...
    # 创建数据集
    # 根据是否使用 HISTORY 模式决定是否传递 num_history_frames 参数
    dataset_kwargs = dict(
        data_root=data_root,
    ...
    # 只有 HISTORY 模式才需要传递 num_history_frames 参数
    if "HISTORY" in wan_type:
        dataset_kwargs['num_history_frames'] = inference_engine.num_history_frames

    dataset = HeatmapDatasetFactory.create_robot_trajectory_dataset(**dataset_kwargs)
    ...
    for idx, dataset_idx in enumerate(test_indices):
        ...
        try:
            ...
            # 提取多帧历史数据（如果可用）
            num_history = sample.get('num_history_frames', 1)
            input_images = sample.get('input_images', None) if num_history > 1 else None  # List[List[PIL.Image]] (num_history, num_views)
            input_images_rgb = sample.get('input_images_rgb', None) if num_history > 1 else None  # List[List[PIL.Image]] (num_history, num_views)
            ...
            if input_images is not None:
                print(f"  Multi-frame history mode: num_history={num_history}, input_images shape=[{len(input_images)}, {len(input_images[0])}]")

            # 执行推理
            # 使用 sequence_length 作为 num_frames 以匹配数据集
            # num_frames = 历史帧数 + 预测帧数（包括初始帧）
            # 当 num_history > 1 时，总帧数应该是 num_history + sequence_length
            if num_history > 1:
                total_frames = num_history + sequence_length
            else:
                total_frames = sequence_length + 1  # 单帧模式：初始帧 + 预测帧

            output = inference_engine.predict(
                prompt=prompt,
                input_image=input_image,
                input_image_rgb=input_image_rgb,
                input_images=input_images,  # 多帧历史热力图（如果可用）
                input_images_rgb=input_images_rgb,  # 多帧历史RGB图像（如果可用）
                initial_rotation=start_rotation_degrees,
                initial_gripper=start_gripper.item(),
                num_frames=total_frames,  # 历史帧 + 预测帧
                height=image_size[0],
                width=image_size[1],
                num_inference_steps=num_inference_steps,
                cfg_scale=cfg_scale,
            ...
            # 注意：predict 函数已经在内部处理了历史帧，返回的预测只包含未来帧
            # 因此这里不需要再进行切片
            pred_rotation_degrees = output['rotation_predictions']  # (sequence_length, 3)
            pred_gripper = output['gripper_predictions']  # (sequence_length,)

            print(f"  Prediction shapes: rotation={pred_rotation_degrees.shape}, gripper={pred_gripper.shape}")
            print(f"  GT shapes: rotation={gt_rotation_degrees.shape}, gripper={gt_gripper_targets.shape}")

            # 计算误差
            # 旋转误差：计算每个角度的绝对误差
            rotation_errors = np.abs(pred_rotation_degrees - gt_rotation_degrees)  # (sequence_length, 3)
            ...
            if num_history > 1:
                # 用于可视化和峰值比较：只使用未来帧
                gt_heatmap_video_eval = gt_heatmap_video[num_history:]
                gt_rgb_video_eval = gt_rgb_video[num_history:]

                # 用于GT latent预测：需要包含最后一个历史帧作为初始帧
                # 这样才能预测正确数量的变化 (13帧 -> 12个预测)
                gt_heatmap_video_with_init = gt_heatmap_video[num_history-1:]
                gt_rgb_video_with_init = gt_rgb_video[num_history-1:]
            else:
                gt_heatmap_video_eval = gt_heatmap_video
                gt_rgb_video_eval = gt_rgb_video
                gt_heatmap_video_with_init = gt_heatmap_video
                gt_rgb_video_with_init = gt_rgb_video
            ...
            # 注意：pipeline 返回的视频帧包含完整序列（历史帧+未来帧）
            # 但 predict 函数内部的 rot_grip_predictor 只预测未来帧
            # 因此视频帧需要切片以匹配 GT
            if num_history > 1:
                # 检测格式并切片：只保留与预测对应的帧
                # 需要保留 sequence_length+1 帧（初始帧+sequence_length个预测帧）
                if isinstance(pred_heatmap[0], list):
                    # 格式是 [view][time]，切片每个view的时间维度
                    # 从最后一个历史帧开始（作为初始帧）
                    pred_heatmap = [view_frames[num_history-1:] for view_frames in pred_heatmap]
                    pred_rgb = [view_frames[num_history-1:] for view_frames in pred_rgb]
                else:
                    # 格式是 [time][view]，直接切片时间维度
                    pred_heatmap = pred_heatmap[num_history-1:]
                    pred_rgb = pred_rgb[num_history-1:]
            ...
            # 计算位置误差（与gt_position_trajectory比较，它是sequence_length+1帧）
            ...
            colormap_name = 'jet'  # 使用与训练相同的colormap（统一使用cv2 JET）

            # 批量计算所有峰值位置（使用评估视频，去除历史帧）
            gt_peaks = inference_engine.find_peaks_batch(gt_heatmap_video_eval, colormap_name)
            pred_peaks = inference_engine.find_peaks_batch(pred_heatmap_video, colormap_name)

            # 计算距离 - 使用两者中较小的长度以避免索引越界
            # GT可能比pred少一帧（因为pred包含初始帧）
            ...
            # 使用包含初始帧的视频，这样可以预测正确数量的未来帧
            gt_latent_output = inference_engine.predict_from_gt_latents(
                gt_rgb_video=gt_rgb_video_with_init,
                gt_heatmap_video=gt_heatmap_video_with_init,
                initial_rotation=start_rotation_degrees,
                initial_gripper=start_gripper.item(),
            ...
            # 保存可视化（类似参考文件的格式）
            # 使用评估视频（去除历史帧）以匹配rotation/gripper targets
            ...
            visualize_predictions_with_rot_grip(
                gt_heatmap_video=gt_heatmap_video_eval,
                pred_heatmap_video=pred_heatmap_video,
                gt_rgb_video=gt_rgb_video_eval,
            ...
            num_views = len(gt_heatmap_video_eval[0])
            num_frames = len(gt_heatmap_video_eval)
...
def main():
    ...
    parser.add_argument("--is_full_finetune", action='store_true', help='Whether the checkpoint is from full finetuning (not LoRA)')
    ...
    parser.add_argument("--local_feat_size", type=int, default=15, help="Local feature size for rotation/gripper predictor")
    ...
    parser.add_argument("--use_different_projection", action='store_true',
                       help='Use different projection mode (3cam_different_projection, must match training)')
    parser.add_argument("--use_initial_gripper_state", action='store_true',
                       help='Use initial gripper state as input (must match training configuration)')
