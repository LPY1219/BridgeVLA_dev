...
from modules.diffusion_action_decoder import DiffusionActionDecoder
...
def parse_args():
    ...
    # Ground truth latents testing (NEW)
    parser.add_argument("--use_gt_latents", action="store_true",
                        help="Use ground truth latents for all frames instead of first frame + noise")
...
@torch.no_grad()
def evaluate_sample(sample, feature_extractor, decoder, args, torch_dtype, use_gt_for_this_call=False):
    """
    Evaluate a single sample using denoising-based feature extraction.

    Args:
        use_gt_for_this_call: If True, encode ALL frames from ground truth (GT latents mode).
                              If False, only encode first frame + noise for future frames (denoising mode).

    Key differences from the original evaluate_sample:
    1. Only encode the first frame from ground truth (when use_gt_for_this_call=False)
    2. Initialize future frames as pure noise (when use_gt_for_this_call=False)
    ...
    # ========================================================================
    # ENCODING MODE SELECTION: GT latents vs First frame + Noise
    # ========================================================================

    if use_gt_for_this_call:
        # ====================================================================
        # MODE 1: Use Ground Truth Latents (encode ALL frames)
        # Reference: inference_diffusion_action_decoder.py lines 238-259
        # ====================================================================
        # Step 1: Encode RGB videos per view
        all_rgb_latents = []
        for v in range(num_views):
            view_frames = [input_video_rgb[t][v] for t in range(num_total_frames)]
            view_video = preprocess_video(view_frames, torch_dtype, args.device).squeeze(0)  # (C, T+1, H, W)
            latents = feature_extractor.pipeline.vae.encode([view_video], device=args.device, tiled=False)
            latents = latents[0].to(dtype=torch_dtype, device=args.device)
            all_rgb_latents.append(latents)

        rgb_latents = torch.stack(all_rgb_latents, dim=0)  # (num_views, c, T+1, h, w)

        # Step 2: Encode Heatmap videos per view
        all_heatmap_latents = []
        for v in range(num_views):
            view_frames = [video_frames[t][v] for t in range(num_total_frames)]
            view_video = preprocess_video(view_frames, torch_dtype, args.device).squeeze(0)  # (C, T+1, H, W)
            latents = feature_extractor.pipeline.vae.encode([view_video], device=args.device, tiled=False)
            latents = latents[0].to(dtype=torch_dtype, device=args.device)
        ...
        # Step 3: Separately encode and replace first frame (align with ImageEmbedderFused)
        # Reference: inference_diffusion_action_decoder.py lines 262-279
        for v in range(num_views):
            # RGB first frame
            ...
            # Replace first frame (align with training behavior)
            rgb_latents[v, :, 0:1, :, :] = first_rgb_latent
            heatmap_latents[v, :, 0:1, :, :] = first_heatmap_latent

    else:
        # ====================================================================
        # MODE 2: First Frame + Noise (denoising mode)
        # ====================================================================
    ...
    # Text encoding (use task instruction from dataset, aligned with training)
    # CRITICAL: Use actual task instruction instead of empty string
    prompt = sample.get('prompt', '')  # Get task instruction from dataset
    if not prompt:
        prompt = ""  # Fallback to empty string if no instruction

    text_embeddings = feature_extractor.pipeline.prompter.encode_prompt(prompt, device=args.device)
...
def main():
    ...
    if args.use_gt_latents:
        print("  Mode: COMPARISON (GT Latents vs Denoising)")
        print("  - GT Latents: Encode all frames from ground truth")
        print("  - Denoising: First frame + noise")
    else:
        print("  Mode: Denoising only (First frame + noise)")
    print(f"  Denoising steps: {args.num_denoising_steps}/{args.num_inference_steps}")
    print("=" * 80)

    # Run inference
    if args.use_gt_latents:
        # Comparison mode: evaluate with both GT latents and denoising
        all_metrics_gt = []
        all_predictions_gt = []
        all_metrics_denoising = []
        all_predictions_denoising = []

        for idx in tqdm(range(num_samples), desc="Evaluating (Comparison Mode)"):
            sample = test_dataset[idx]

            # Mode 1: GT latents (encode all frames)
            metrics_gt, predictions_gt = evaluate_sample(
                sample, feature_extractor, decoder, args, torch_dtype,
                use_gt_for_this_call=True
            )
            all_metrics_gt.append(metrics_gt)
            if predictions_gt:
                all_predictions_gt.append({'sample_idx': idx, **predictions_gt})

            # Mode 2: Denoising (first frame + noise)
            metrics_denoising, predictions_denoising = evaluate_sample(
                sample, feature_extractor, decoder, args, torch_dtype,
                use_gt_for_this_call=False
            )
            all_metrics_denoising.append(metrics_denoising)
            if predictions_denoising:
                all_predictions_denoising.append({'sample_idx': idx, **predictions_denoising})
    else:
        # Normal mode: only denoising
        all_metrics = []
        all_predictions = []

        for idx in tqdm(range(num_samples), desc="Evaluating"):
            sample = test_dataset[idx]
            metrics, predictions = evaluate_sample(
                sample, feature_extractor, decoder, args, torch_dtype,
                use_gt_for_this_call=False
            )
    ...
    print("=" * 80)
    if args.use_gt_latents:
        print("EVALUATION RESULTS (COMPARISON MODE)")
        print("=" * 80)

        # Compute average metrics for GT latents
        avg_metrics_gt = {}
        for key in all_metrics_gt[0].keys():
            avg_metrics_gt[key] = np.mean([m[key] for m in all_metrics_gt])

        # Compute average metrics for denoising
        avg_metrics_denoising = {}
        for key in all_metrics_denoising[0].keys():
            avg_metrics_denoising[key] = np.mean([m[key] for m in all_metrics_denoising])

        # Print GT latents results
        print("\n--- MODE 1: Ground Truth Latents (All frames encoded) ---")
        for key in all_metrics_gt[0].keys():
            avg_val = avg_metrics_gt[key]
            std_val = np.std([m[key] for m in all_metrics_gt])
            print(f"{key}: {avg_val:.4f} ± {std_val:.4f}")

        # Print denoising results
        print("\n--- MODE 2: Denoising (First frame + noise) ---")
        for key in all_metrics_denoising[0].keys():
            avg_val = avg_metrics_denoising[key]
            std_val = np.std([m[key] for m in all_metrics_denoising])
            print(f"{key}: {avg_val:.4f} ± {std_val:.4f}")

        # Print comparison (difference)
        print("\n--- COMPARISON (GT Latents - Denoising) ---")
        for key in all_metrics_gt[0].keys():
            diff = avg_metrics_gt[key] - avg_metrics_denoising[key]
            # For accuracy: higher is better, so positive diff means GT is better
            # For error (e.g., heatmap_error_pixels): lower is better, so negative diff means GT is better
            if 'accuracy' in key:
                indicator = "(GT better)" if diff > 0 else "(Denoising better)" if diff < 0 else "(Same)"
            elif 'error' in key:
                indicator = "(GT better)" if diff < 0 else "(Denoising better)" if diff > 0 else "(Same)"
            else:
                indicator = ""
            print(f"Δ {key}: {diff:+.4f} {indicator}")
        ...
        results = {
            ...
            'num_inference_steps': args.num_inference_steps,
            'gt_latents_results': {
                'avg_metrics': avg_metrics_gt,
                'all_metrics': all_metrics_gt,
                'predictions': all_predictions_gt if all_predictions_gt else None,
            },
            'denoising_results': {
                'avg_metrics': avg_metrics_denoising,
                'all_metrics': all_metrics_denoising,
                'predictions': all_predictions_denoising if all_predictions_denoising else None,
            },
            'comparison': {
                key: avg_metrics_gt[key] - avg_metrics_denoising[key]
                for key in all_metrics_gt[0].keys()
            }
        }

        results_path = os.path.join(args.output_dir, 'inference_results_comparison.json')
    else:
