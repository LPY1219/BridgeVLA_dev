"""
Inference script for Diffusion Action Decoder

This script evaluates a trained action decoder on test data by:
1. Loading a trained action decoder checkpoint
2. Running inference on test trajectories
3. Comparing predictions vs ground truth
4. Computing and reporting metrics

Key design:
- Uses the same pipeline loading as training (for compatibility)
- Fixed denoising timestep for consistent evaluation
- Reports per-sample and aggregate metrics

Usage:
    python inference_diffusion_action_decoder.py \
...
def parse_args():
    parser = argparse.ArgumentParser(description="Action Decoder Inference")
    ...
    parser.add_argument("--denoising_timestep_id", type=int, required=True)
...
def evaluate_sample(sample, feature_extractor, decoder, args, torch_dtype):
    ...
    # VAE encoding (fully aligned with training script)
    ...
    # This is CRITICAL for alignment with training behavior!
    # Reference: train_diffusion_action_decoder.py lines 233-255
    for v in range(num_views):
        # RGB first frame
        ...
        # Replace first frame (align with training behavior: latents[:, :, 0:1] = z_fused)
    ...
    dit_output = feature_extractor(
        rgb_latents=rgb_latents,
        heatmap_latents=heatmap_latents,
        text_embeddings=text_embeddings,
        denoising_timestep_id=args.denoising_timestep_id,
    )
    ...
    # Training normalizes delta to image size, so we must denormalize for evaluation
    ...
    # Compute ground truth (fully aligned with training script)
    # Heatmap peaks - img_locations shape: (num_poses, num_views, 2) or possibly (1, num_poses, num_views, 2)
    # num_poses = 1 (initial) + num_future_frames

    # Remove extra batch dimension if present (aligned with training)
    if img_locations.dim() == 4:
        img_locations = img_locations.squeeze(0)  # (num_poses, num_views, 2)

    # Strict assertion: verify dimensions match configuration
    num_future_frames = args.num_future_frames
    expected_num_poses = 1 + num_future_frames  # Should be SEQUENCE_LENGTH
    ...
    # CRITICAL: Keep all views, don't average
    initial_peaks = img_locations[0, :, :]  # (num_views, 2) - all views initial frame
    future_peaks = img_locations[1:, :, :]  # (num_future_frames, num_views, 2) - all views future frames
    heatmap_delta_gt = future_peaks - initial_peaks.unsqueeze(0)  # (num_future_frames, num_views, 2) in pixels

    # Rotation bins (aligned with training script)
    initial_quat = start_pose[3:]  # (4,) xyzw
    future_quats = future_poses[:, 3:]  # (T, 4)

    T = len(future_poses)

    # Convert to Euler angles (aligned with training)
    initial_euler = quaternion_to_euler(initial_quat.unsqueeze(0)).squeeze(0)  # (3,)
    future_euler = quaternion_to_euler(future_quats)  # (T, 3)

    # Compute rotation delta (aligned with training)
    rotation_delta = compute_rotation_delta(
        initial_euler.unsqueeze(0).expand(T, -1), future_euler
    )  # (T, 3)

    # Convert to bins (aligned with training)
    ...
    # Compute metrics
    # Heatmap error (L2 pixels) - compute per view then average
    # heatmap_delta_pred: (1, T, num_views, 2)
    # heatmap_delta_gt: (T, num_views, 2)
    hm_delta_diff = heatmap_delta_pred.squeeze(0) - heatmap_delta_gt  # (T, num_views, 2)
    hm_err = torch.norm(hm_delta_diff, dim=2)  # (T, num_views) - L2 error per frame per view
    mean_hm_err = hm_err.mean().item()  # Average across all frames and views
...
def main():
    ...
    # Initialize scheduler timesteps (CRITICAL for inference!)
    # Must match training: 1000 timesteps so timestep_id can be in range [0, 1000)
    # Reference: train_diffusion_action_decoder.py line 459
    ...
    print(f"\nEvaluating {num_samples} samples (timestep_id={args.denoising_timestep_id})...")
    print("=" * 80)
