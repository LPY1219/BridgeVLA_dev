...
class WanModel(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        print("[DEBUG] WanModel initialized from: wan_video_dit_fused.py")
        self.dim = dim
        self.in_dim = in_dim
    ...
    def patchify(self, x: torch.Tensor, control_camera_latents_input: Optional[torch.Tensor] = None):
        # DEBUG: Verify this is wan_video_dit_fused.py
        print(f"[DEBUG] patchify called from: {__file__}")
    ...
    def adapt_pretrained_weights(self, pretrained_state_dict, strict=False):
        ...
        Returns:
            加载结果信息
        """
        print(f"        [WanModel] adapt_pretrained_weights called")
        ...
        if 'patch_embedding.weight' in pretrained_state_dict:
            ...
            if pretrained_weight.shape != current_weight.shape:
                ...
                if current_in_dim == 2 * pretrained_in_dim:
                    # 输入维度翻倍：沿输入通道维度复制权重
                    adapted_weight = torch.cat([pretrained_weight, pretrained_weight], dim=1)
                    # 对复制的权重进行缩放，保持总体输入能量一致
                    adapted_weight = adapted_weight * 0.5
                    adapted_state_dict['patch_embedding.weight'] = adapted_weight
                    size_mismatched_keys.append('patch_embedding.weight')
                    print("  -> Duplicated and scaled input channels")
                elif current_in_dim > pretrained_in_dim:
                    # 任意扩展：用零初始化新通道
                    adapted_weight = torch.zeros_like(current_weight)
                    adapted_weight[:, :pretrained_in_dim] = pretrained_weight
                    adapted_state_dict['patch_embedding.weight'] = adapted_weight
                    size_mismatched_keys.append('patch_embedding.weight')
                    print(f"  -> Zero-initialized additional {current_in_dim - pretrained_in_dim} channels")
                else:
                    # 维度缩小：截断
        ...
        if 'head.head.weight' in pretrained_state_dict:
            ...
            if pretrained_weight.shape != current_weight.shape:
                ...
                if current_out_dim == 2 * pretrained_out_dim:
                    # 输出维度翻倍：沿输出维度复制权重
                    adapted_weight = torch.cat([pretrained_weight, pretrained_weight], dim=0)
                    adapted_state_dict['head.head.weight'] = adapted_weight
                    size_mismatched_keys.append('head.head.weight')
                    print("  -> Duplicated output features")
                elif current_out_dim > pretrained_out_dim:
                    # 任意扩展：用小随机值初始化新通道
                    adapted_weight = torch.zeros_like(current_weight)
                    adapted_weight[:pretrained_out_dim] = pretrained_weight
                    # 对新通道使用小的随机初始化
                    nn.init.normal_(adapted_weight[pretrained_out_dim:], mean=0.0, std=0.02)
                    adapted_state_dict['head.head.weight'] = adapted_weight
                    size_mismatched_keys.append('head.head.weight')
                    print(f"  -> Initialized additional {current_out_dim - pretrained_out_dim} output features")
                else:
                    # 维度缩小：截断
        ...
        if 'head.head.bias' in pretrained_state_dict:
            ...
            if pretrained_bias.shape != current_bias.shape:
                ...
                if current_out_dim == 2 * pretrained_out_dim:
                    # 输出维度翻倍：复制 bias
                    adapted_bias = torch.cat([pretrained_bias, pretrained_bias], dim=0)
        ...
        # 5. 加载适配后的权重
        load_result = self.load_state_dict(adapted_state_dict, strict=False)
