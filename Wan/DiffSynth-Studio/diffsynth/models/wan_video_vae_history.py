...
class VideoVAE_(nn.Module):
    ...
    def encode(self, x, scale):
        ...
        Returns:
            编码后的mu张量，shape为 (B, z_dim, T_latent, H//8, W//8)
            其中 T_latent = 1 + ceil((T-1)/4)
        """
        self.clear_cache()
        ## cache
        t = x.shape[2]
...
class WanVideoVAE(nn.Module):
    ...
    def tiled_encode(self, video, device, tile_size, tile_stride, num_condition_frames=1):
        """
        Tiled编码模式
        ...
        remaining_frames = T - num_cond
        out_T = 1 + (remaining_frames + 3) // 4 if remaining_frames > 0 else 1
        ...
        for h, h_, w, w_ in tqdm(tasks, desc="VAE encoding"):
            ...
            hidden_states_batch = self.model.encode(hidden_states_batch, self.scale, num_condition_frames=num_condition_frames).to(data_device)
    ...
    def single_encode(self, video, device, num_condition_frames=1):
        """
        单次编码（非tiled模式）
    ...
    def encode(self, videos, device, tiled=False, tile_size=(34, 34), tile_stride=(18, 16), num_condition_frames=1):
        """
        编码视频列表
        ...
        Args:
            ...
            tile_stride: tile步长
            num_condition_frames: 条件帧数量（1-4）。
                当 num_condition_frames=1 时，与原有行为一致（向后兼容）
                当 num_condition_frames>1 时，前 num_condition_frames 帧作为条件一起编码
        """
        ...
        for video in videos:
            ...
            if tiled:
                ...
                hidden_state = self.tiled_encode(video, device, tile_size, tile_stride, num_condition_frames=num_condition_frames)
            else:
                hidden_state = self.single_encode(video, device, num_condition_frames=num_condition_frames)
...
class VideoVAE38_(VideoVAE_):
    ...
    def encode(self, x, scale):
        """
        编码视频为latent空间表示（带patchify的版本，用于VideoVAE38_，与原始VAE一致）
