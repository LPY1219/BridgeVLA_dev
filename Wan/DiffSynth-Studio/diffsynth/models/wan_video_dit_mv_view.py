...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs, freqs_mvs, shape_info):
        ...
        # Debug: 只打印一次t_mod形状信息
        if not hasattr(self, '_t_mod_shape_printed'):
            # print(f"[DEBUG] WanBlockMV.forward: t_mod.shape={t_mod.shape}, has_seq={has_seq}")
            if not has_seq:
                print(f"[WARNING] has_seq=False may cause gate_mvs rearrange error. Ensure dit.seperated_timestep=True and fuse_vae_embedding_in_latents=True")
            self._t_mod_shape_printed = True
...
class WanModel_mv_view(torch.nn.Module):
    def __init__(
        self,
        dim: int,
        in_dim: int,
        ...
        super().__init__()
        print("[DEBUG] WanModel_mv initialized from: wan_video_dit_mv.py")
        self.dim = dim
        self.in_dim = in_dim
        ...
        # 根据dual head模式选择patch_embedding结构
        if use_dual_head:
            # 双head模式：RGB和Heatmap各自有独立的patch_embedding
            # RGB views (前3个) 通过 patch_embedding_rgb
            # Heatmap views (后3个) 通过 patch_embedding_heatmap
            self.patch_embedding_rgb = nn.Conv3d(
                in_dim, dim, kernel_size=patch_size, stride=patch_size)
            self.patch_embedding_heatmap = nn.Conv3d(
                in_dim, dim, kernel_size=patch_size, stride=patch_size)
            self.patch_embedding = None  # 不使用单一patch_embedding
        else:
            # 单head模式：使用共享的patch_embedding
    ...
    def patchify(self, x: torch.Tensor, control_camera_latents_input: Optional[torch.Tensor] = None):
        ...
        if self.use_dual_head:
            # Dual head mode: separate processing for RGB and Heatmap views
            # Input x shape: (6, 48, f, h, w) where first 3 views are RGB, last 3 are Heatmap
            num_views = x.shape[0]
            num_rgb_views = num_views // 2

            x_rgb = x[:num_rgb_views]  # (3, 48, f, h, w)
            x_heatmap = x[num_rgb_views:]  # (3, 48, f, h, w)

            # Process RGB views through patch_embedding_rgb
            x_rgb = self.patch_embedding_rgb(x_rgb)
            # Process Heatmap views through patch_embedding_heatmap
            x_heatmap = self.patch_embedding_heatmap(x_heatmap)

            # Concatenate along batch dimension: (6, dim, f', h', w')
            x = torch.cat([x_rgb, x_heatmap], dim=0)
        else:
            # Single head mode: shared patch_embedding
    ...
    def forward(self,
        ...
        # 实际训练过程中其实并没有用到这个forward，而是自定义了前向的过程。
        ...
        if self.use_dual_head:
            # 双head模式：分离RGB和Heatmap views，分别通过对应的head
            # x shape: (6, num_patches, dim) where first 3 are RGB, last 3 are Heatmap
            num_views = x.shape[0]
            num_rgb_views = num_views // 2

            x_rgb = x[:num_rgb_views]  # (3, num_patches, dim)
            x_heatmap = x[num_rgb_views:]  # (3, num_patches, dim)

            # RGB views through head_rgb
            x_rgb = self.head_rgb(x_rgb, t)
            x_rgb = self.unpatchify(x_rgb, (f, h, w))  # (3, 48, f, h, w)

            # Heatmap views through head_heatmap
            x_heatmap = self.head_heatmap(x_heatmap, t)
            x_heatmap = self.unpatchify(x_heatmap, (f, h, w))  # (3, 48, f, h, w)

            # Concatenate along batch/view dimension: (6, 48, f, h, w)
            # NOT along channel dimension - keeping views separate
            x = torch.cat([x_rgb, x_heatmap], dim=0)
            ...
            if not hasattr(self, '_dual_head_debug_printed'):
                ...
                if local_rank == 0:
                    print(f"[DEBUG] WanModel_mv Forward (View-wise): Using DUAL HEAD with DUAL PATCH_EMBEDDING")
                    print(f"        x_rgb.shape={x_rgb.shape}, x_heatmap.shape={x_heatmap.shape}")
                    print(f"        output.shape={x.shape} (6 views, 48 channels each)")
    ...
    def adapt_pretrained_weights(self, pretrained_state_dict, strict=False):
        ...
        if is_main_process:
            print("        [WanModel_mv_view] adapt_pretrained_weights called")
            print(f"        [WanModel_mv_view] Current in_dim: {self.in_dim}, use_dual_head: {self.use_dual_head}")
        ...
        # 0.5 处理双patch_embedding模式的特殊情况
        if self.use_dual_head:
            # 在dual head模式下，需要将预训练的patch_embedding权重复制到patch_embedding_rgb和patch_embedding_heatmap
            if is_main_process:
                print("\n" + "="*80)
                print("[DEBUG] DUAL PATCH_EMBEDDING MODE: Initializing patch_embedding_rgb and patch_embedding_heatmap")
                print("="*80)

            # 处理 patch_embedding.weight -> patch_embedding_rgb.weight 和 patch_embedding_heatmap.weight
            if 'patch_embedding.weight' in pretrained_state_dict:
                pretrained_patch_weight = pretrained_state_dict['patch_embedding.weight']
                if is_main_process:
                    print(f"[DEBUG] Copying patch_embedding.weight to both patch_embeddings")
                    print(f"  Source shape: {pretrained_patch_weight.shape}")
                adapted_state_dict['patch_embedding_rgb.weight'] = pretrained_patch_weight.clone()
                adapted_state_dict['patch_embedding_heatmap.weight'] = pretrained_patch_weight.clone()
                if is_main_process:
                    print(f"  ✓ patch_embedding_rgb.weight initialized (shape: {pretrained_patch_weight.shape})")
                    print(f"  ✓ patch_embedding_heatmap.weight initialized (shape: {pretrained_patch_weight.shape})")

            # 处理 patch_embedding.bias
            if 'patch_embedding.bias' in pretrained_state_dict:
                pretrained_patch_bias = pretrained_state_dict['patch_embedding.bias']
                adapted_state_dict['patch_embedding_rgb.bias'] = pretrained_patch_bias.clone()
                adapted_state_dict['patch_embedding_heatmap.bias'] = pretrained_patch_bias.clone()
                if is_main_process:
                    print(f"  ✓ Both patch_embedding biases initialized (shape: {pretrained_patch_bias.shape})")

            if is_main_process:
                print("="*80)
                print("[DEBUG] Dual Patch_Embedding Initialization Complete!")
                print("="*80 + "\n")

        # 1. 处理 patch_embedding 权重 (输入维度可能不同) - 仅在非dual_head模式下
        if not self.use_dual_head and 'patch_embedding.weight' in pretrained_state_dict:
        ...
        # 处理 patch_embedding bias - 仅在非dual_head模式下
        if not self.use_dual_head and 'patch_embedding.bias' in pretrained_state_dict:
