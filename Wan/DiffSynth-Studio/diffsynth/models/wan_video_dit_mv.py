...
class WanModel_mv(torch.nn.Module):
    def __init__(
        self,
        dim: int,
        in_dim: int,
    ...
    def forward(self,
        ...
        if self.use_dual_head:
            ...
            if not hasattr(self, '_dual_head_debug_printed'):
                ...
                if local_rank == 0:
                    print(f"[DEBUG] WanModel_mv Forward: Using DUAL HEAD - x_rgb.shape={x_rgb.shape}, x_heatmap.shape={x_heatmap.shape}, output.shape={x.shape}")
    ...
    def adapt_pretrained_weights(self, pretrained_state_dict, strict=False):
        ...
        if is_main_process:
            print("        [WanModel_mv] adapt_pretrained_weights called")
            print(f"        [WanModel_mv] Current in_dim: {self.in_dim}")
