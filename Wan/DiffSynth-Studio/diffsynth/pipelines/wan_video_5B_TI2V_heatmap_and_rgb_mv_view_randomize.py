...
from ..models.wan_video_dit_mv_view import WanModel_mv_view, RMSNorm, sinusoidal_embedding_1d
...
def convert_wan_model_to_mv(wan_model, use_dual_head: bool = False):
    """
    将 WanModel 转换为 WanModel_mv_view，保留所有权重
    ...
    print(f"    Creating WanModel_mv_view with config: dim={config['dim']}, in_dim={config['in_dim']}, "
          f"out_dim={config['out_dim']}, num_layers={config.get('num_layers', 'N/A')}, "
          f"use_dual_head={use_dual_head}")

    # 创建新的 WanModel_mv_view 实例
    wan_model_mv = WanModel_mv_view(**config)

    # 复制所有权重
    original_state_dict = wan_model.state_dict()

    # 如果使用 dual head，需要特殊处理 patch_embedding 和 head 权重
    if use_dual_head:
        # 复制所有权重，并特殊处理 patch_embedding 和 head
        new_state_dict = {}
        has_single_head = 'head.head.weight' in original_state_dict
        has_dual_head = 'head_rgb.head.weight' in original_state_dict

        for key, value in original_state_dict.items():
            # 处理 patch_embedding 权重复制（WanModel只有单个patch_embedding）
            if key == 'patch_embedding.weight':
                new_state_dict['patch_embedding_rgb.weight'] = value.clone()
                new_state_dict['patch_embedding_heatmap.weight'] = value.clone()
                print(f"    ✓ Duplicated patch_embedding.weight -> patch_embedding_rgb/heatmap.weight")
            elif key == 'patch_embedding.bias':
                new_state_dict['patch_embedding_rgb.bias'] = value.clone()
                new_state_dict['patch_embedding_heatmap.bias'] = value.clone()
                print(f"    ✓ Duplicated patch_embedding.bias -> patch_embedding_rgb/heatmap.bias")
            # 处理单head模式的权重复制（WanModel初始化为单head）
            elif has_single_head and key.startswith('head.'):
                if key.startswith('head.head.'):
                    ...
                    new_state_dict[new_key_rgb] = value.clone()
                    new_state_dict[new_key_heatmap] = value.clone()
            # 处理双head模式的权重直接复制（WanModel初始化为dual_head）
            elif has_dual_head and (key.startswith('head_rgb.') or key.startswith('head_heatmap.')):
                new_state_dict[key] = value
            # 其他权重直接复制
            elif not key.startswith('head.') and not key.startswith('patch_embedding.'):
                new_state_dict[key] = value

        wan_model_mv.load_state_dict(new_state_dict, strict=False)
        print("    ✓ Converted to WanModel_mv_view with dual head and dual patch_embedding")
        print("      (patch_embedding weights duplicated, head weights copied/duplicated)")
    else:
        # 直接加载所有权重
        missing_keys, unexpected_keys = wan_model_mv.load_state_dict(original_state_dict, strict=False)
        if len(missing_keys) > 0:
            print(f"    ⚠️  Missing keys in WanModel_mv_view: {len(missing_keys)} keys")
        ...
        if len(missing_keys) == 0 and len(unexpected_keys) == 0:
            print("    ✓ Successfully converted to WanModel_mv_view (all weights transferred)")
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.dit: WanModel_mv_view = None
        self.dit2: WanModel_mv_view = None
    ...
    def training_loss(self, rgb_loss_weight=0.5, **inputs):
        ...
        if not hasattr(self, '_training_loss_debug_printed'):
            ...
            if local_rank == 0:
                print(f"[DEBUG] Training Loss (View-wise): noise_pred.shape={noise_pred.shape}, training_target.shape={training_target.shape}")
                print(f"[DEBUG] RGB Loss Weight: {rgb_loss_weight}")
            self._training_loss_debug_printed = True

        # View-wise loss computation
        # noise_pred shape: (6, 48, T, H, W) where first 3 are RGB, last 3 are Heatmap
        num_views = noise_pred.shape[0]
        num_rgb_views = num_views // 2

        noise_pred_rgb = noise_pred[:num_rgb_views]  # (3, 48, T, H, W)
        noise_pred_heatmap = noise_pred[num_rgb_views:]  # (3, 48, T, H, W)
        training_target_rgb = training_target[:num_rgb_views]  # (3, 48, T, H, W)
        training_target_heatmap = training_target[num_rgb_views:]  # (3, 48, T, H, W)
        ...
        # 总loss = 加权RGB loss + 加权Heatmap loss
        ...
        if not hasattr(self, '_loss_split_debug_printed'):
            ...
            local_rank = int(os.environ.get("LOCAL_RANK", 0))
            if local_rank == 0:
                print(f"[DEBUG] Loss Split (View-wise): RGB views={num_rgb_views}, Heatmap views={num_rgb_views}")
    ...
    def from_pretrained(
        ...
        if tokenizer_config is None:
            ...
            elif os.path.exists("/DATA/disk1/lpy_a100_1/huggingface"):
                base_path="/DATA/disk1/lpy_a100_1/huggingface"
            elif os.path.exists("/mnt/robot-rfm/user/lpy/huggingface"):
                base_path="/mnt/robot-rfm/user/lpy/huggingface"
            else:
            ...
            if wan_type =="5B_TI2V":
                ...
                # Channel concatenation mode: use fused model tokenizer
                tokenizer_config = ModelConfig(path=f"{base_path}/Wan2.2-TI2V-5B-fused/google/umt5-xxl")
            elif wan_type =="5B_TI2V_RGB_HEATMAP_MV":
                # View concatenation mode: use original model tokenizer
                # NOTE: Using original Wan2.2-TI2V-5B tokenizer for view-wise processing
                tokenizer_config = ModelConfig(path=f"{base_path}/Wan2.2-TI2V-5B/google/umt5-xxl")
            elif wan_type =="5B_TI2V_RGB_HEATMAP_MV_ROT_GRIP":
                # View concatenation mode with rotation and gripper prediction
                # Use same tokenizer as 5B_TI2V_RGB_HEATMAP_MV
                tokenizer_config = ModelConfig(path=f"{base_path}/Wan2.2-TI2V-5B/google/umt5-xxl")
        ...
        # 消融实验：重新初始化 DIT 权重（不使用预训练权重）
        if dit is not None:
            print(f"🔬 [Ablation] Reinitializing DIT weights (discarding pretrained weights)")

            def reinit_weights(module):
                """重新初始化模块的权重"""
                if hasattr(module, 'weight') and module.weight is not None:
                    if len(module.weight.shape) >= 2:  # Linear/Conv layers
            ...
            if isinstance(dit, list):
                for single_dit in dit:
                    if single_dit is not None:
                        single_dit.apply(reinit_weights)
                        print(f"    ✓ Reinitialized DIT model in list")
            else:
                dit.apply(reinit_weights)
                print(f"    ✓ Reinitialized DIT model")

        # Convert WanModel to WanModel_mv_view for MV support
        # When wan_type contains "_MV", we need to use WanModel_mv_view which has multi-view forward logic
        if wan_type and "_MV" in wan_type:
            print(f"🔄 Converting DIT model from WanModel to WanModel_mv_view for {wan_type}")
            if isinstance(dit, list):
                converted_dit_list = []
                for single_dit in dit:
        ...
        if os.path.exists(config_path):
            ...
            if pipe.dit and hasattr(pipe.dit, 'in_dim'):
                ...
                if need_adaptation:
                    ...
                    # 消融实验：不加载预训练权重，直接使用空字典
                    print(f"🔬 [Ablation] Using empty state dict (no pretrained weights for dimension adaptation)")
                    dit_state_dict = {}  # 空字典，不加载任何预训练权重
    ...
    def __call__(
        ...
        if input_image is not None and isinstance(input_image, list):
            num_view_base = len(input_image)
        else:
            num_view_base = 3

        # In view-concatenation mode with both RGB and heatmap inputs, we need double the views
        # because RGB and heatmap views are concatenated along batch dimension
        # Example: 3 heatmap views + 3 RGB views = 6 total views in batch dimension
        if input_image_rgb is not None and isinstance(input_image_rgb, list):
            num_view = num_view_base * 2  # Double for RGB + Heatmap concatenation
        else:
            num_view = num_view_base
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if "first_frame_latents" in inputs_shared and  "first_frame_latents_rgb" in inputs_shared:
                # View concatenation: concatenate along view dimension (dim=0)
                # first_frame_latents_rgb: (3, 48, 1, H, W), first_frame_latents: (3, 48, 1, H, W) → fused: (6, 48, 1, H, W)
                first_frame_latents_fused= torch.cat((inputs_shared["first_frame_latents_rgb"],inputs_shared["first_frame_latents"]),dim=0)
        ...
        num_views = inputs_shared["latents"].shape[0]
        num_channels = inputs_shared["latents"].shape[1]

        # View concatenation mode: latents shape is (6, 48, T, H, W)
        # First 3 views are RGB, last 3 views are Heatmap
        assert num_views == 6, f"Expected 6 views, got {num_views}"
        assert num_channels == 48, f"Expected 48 channels per view, got {num_channels}"

        # Decode RGB views (views 0, 1, 2)
        video_rgb_1 = self.vae.decode(inputs_shared["latents"][0:1], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_rgb_1 = self.vae_output_to_video(video_rgb_1)

        video_rgb_2 = self.vae.decode(inputs_shared["latents"][1:2], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_rgb_2 = self.vae_output_to_video(video_rgb_2)

        video_rgb_3 = self.vae.decode(inputs_shared["latents"][2:3], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_rgb_3 = self.vae_output_to_video(video_rgb_3)

        # Decode Heatmap views (views 3, 4, 5) with inverse scaling
        heatmap_latents_1 = inputs_shared["latents"][3:4]
        if HEATMAP_LATENT_SCALE_FACTOR != 1.0:
            heatmap_latents_1 = heatmap_latents_1 / HEATMAP_LATENT_SCALE_FACTOR
        video_heatmap_1 = self.vae.decode(heatmap_latents_1, device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_heatmap_1 = self.vae_output_to_video(video_heatmap_1)

        heatmap_latents_2 = inputs_shared["latents"][4:5]
        if HEATMAP_LATENT_SCALE_FACTOR != 1.0:
            heatmap_latents_2 = heatmap_latents_2 / HEATMAP_LATENT_SCALE_FACTOR
        video_heatmap_2 = self.vae.decode(heatmap_latents_2, device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_heatmap_2 = self.vae_output_to_video(video_heatmap_2)

        heatmap_latents_3 = inputs_shared["latents"][5:6]
        if HEATMAP_LATENT_SCALE_FACTOR != 1.0:
            heatmap_latents_3 = heatmap_latents_3 / HEATMAP_LATENT_SCALE_FACTOR
        video_heatmap_3 = self.vae.decode(heatmap_latents_3, device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_heatmap_3 = self.vae_output_to_video(video_heatmap_3)    
        

        self.load_models_to_device([])

        # Return as dictionary for consistency with inference code
        # Also return latents for rotation/gripper prediction
        return {
            'video_heatmap': [video_heatmap_1, video_heatmap_2, video_heatmap_3],
            'video_rgb': [video_rgb_1, video_rgb_2, video_rgb_3],
            'rgb_latents': inputs_shared["latents"][0:3],  # Views 0, 1, 2 (RGB)
            'heatmap_latents': inputs_shared["latents"][3:6]  # Views 3, 4, 5 (Heatmap, before inverse scaling)
        }
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, vace_reference_image,num_view):
        ...
        if vace_reference_image is not None:
            length += 1
        # View concatenation mode: num_view will be 6 (3 RGB + 3 Heatmap), so no need to double channels
        shape = (num_view, pipe.vae.model.z_dim, length, height // pipe.vae.upsampling_factor, width // pipe.vae.upsampling_factor)
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image,input_video_rgb):
        ...
        if input_video_rgb is not None:
            ...
            # View concatenation mode: concatenate along batch/view dimension (dim=0) instead of channel dimension
            # This creates 6 views (3 RGB + 3 Heatmap) with 48 channels each
            # Result shape: (6, 48, T, H, W) instead of (3, 96, T, H, W)
            input_latents = torch.concat([input_latents_rgb, input_latents], dim=0) # 沿着view维度拼接
...
class WanVideoUnit_ImageEmbedderFused(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_image, latents, height, width, tiled, tile_size, tile_stride,input_image_rgb):
        ...
        # View concatenation: concatenate along view dimension (dim=0)
        # z_rgb: (3, 48, 1, H, W), z: (3, 48, 1, H, W) → z_fused: (6, 48, 1, H, W)
        z_fused = torch.cat([z_rgb, z], dim=0)
        latents[:, :, 0: 1] = z_fused
...
class TeaCache:
    ...
    def check(self, dit: WanModel_mv_view, x, t_mod):
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    x = latents
    # Save the number of views before any transformations
    num_views = x.shape[0]  # 6 in view concatenation mode (3 RGB + 3 Heatmap)
    ...
    # Use actual number of views (6 in view concatenation mode, 3 in legacy mode)
    v = num_views
    freqs_mvs = torch.cat([
    ...
    if tea_cache_update:
        ...
        if tea_cache is not None:
            tea_cache.store(x)
    if use_dual_head:
        # Split views for dual head processing
        # x: (6, num_patches, dim) with 6 views (3 RGB + 3 Heatmap)
        num_views = x.shape[0]
        num_rgb_views = num_views // 2
        x_rgb_views = x[:num_rgb_views]  # (3, num_patches, dim)
        x_heatmap_views = x[num_rgb_views:]  # (3, num_patches, dim)

        # Process through separate heads
        x_rgb_views = dit.head_rgb(x_rgb_views, t)
        x_heatmap_views = dit.head_heatmap(x_heatmap_views, t)

        # Unpatchify each
        x_rgb = dit.unpatchify(x_rgb_views, (f, h, w))  # (3, 48, f, h, w)
        x_heatmap = dit.unpatchify(x_heatmap_views, (f, h, w))  # (3, 48, f, h, w)

        # View concatenation: concatenate along view dimension (dim=0)
        x = torch.cat((x_rgb, x_heatmap), dim=0)  # (6, 48, f, h, w)
        return x
    else:
        x = dit.head(x, t)
    if use_unified_sequence_parallel:
