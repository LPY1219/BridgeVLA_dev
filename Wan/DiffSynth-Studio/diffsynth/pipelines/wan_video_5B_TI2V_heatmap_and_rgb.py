# 采用第一帧的heatmap作为首帧输入图像，第一帧的rgb作为交叉注意力的条件图像
...
class WanVideoPipeline(BasePipeline):
            ...
            if pipe.dit and hasattr(pipe.dit, 'in_dim'):
                ...
                if need_adaptation:
                    ...
                    if hasattr(pipe.dit, 'adapt_pretrained_weights'):
                        ...
                        print(f"    ✓ DIT adapted to in_dim={pipe.dit.in_dim}, out_dim={config_out_dim if config_out_dim else 'unchanged'}")
    ...
    def __call__(
        ...
        # RGB condition (new)
        condition_rgb: Optional[Image.Image] = None,
        ...
        video_rgb = self.vae.decode(inputs_shared["latents"][:,:num_channels//2], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_rgb = self.vae_output_to_video(video_rgb)
        # Apply inverse scaling to heatmap latents before decoding
        heatmap_latents = inputs_shared["latents"][:,num_channels//2:]
        ...
        return video_heatmap,video_rgb
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, vace_reference_image):
        ...
        if vace_reference_image is not None:
            length += 1
        shape = (1, pipe.vae.model.z_dim*2, length, height // pipe.vae.upsampling_factor, width // pipe.vae.upsampling_factor)
