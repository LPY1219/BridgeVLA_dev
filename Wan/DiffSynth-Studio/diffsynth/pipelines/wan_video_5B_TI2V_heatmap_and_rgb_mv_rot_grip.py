...
def convert_wan_model_to_mv(wan_model, use_dual_head: bool = False):
    """
    将 WanModel 转换为 WanModel_mv，保留所有权重
    ...
    if use_dual_head and 'head.head.weight' in original_state_dict:
        ...
        print("    ✓ Converted to WanModel_mv with dual head (head weights duplicated)")
    else:
        # 直接加载所有权重
        missing_keys, unexpected_keys = wan_model_mv.load_state_dict(original_state_dict, strict=False)
        if len(missing_keys) > 0:
            print(f"    ⚠️  Missing keys in WanModel_mv: {len(missing_keys)} keys")
        ...
        if len(missing_keys) == 0 and len(unexpected_keys) == 0:
            print("    ✓ Successfully converted to WanModel_mv (all weights transferred)")
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        inputs["latents"] = self.scheduler.add_noise(inputs["input_latents"], inputs["noise"], timestep) # 第一帧被noise覆盖掉了？
        training_target = self.scheduler.training_target(inputs["input_latents"], inputs["noise"], timestep) # 这里其实有点没理解？为什么训练目标是最初的噪声减去干净的图像？
        ...
        if not hasattr(self, '_loss_split_debug_printed'):
            ...
            if local_rank == 0:
                ...
                print(f"[DEBUG] loss={loss.item():.4f}, loss_rgb={loss_rgb.item():.4f}, loss_heatmap={loss_heatmap.item():.4f}")
    ...
    def from_pretrained(
        ...
        if wan_type and "_MV" in wan_type:
            print(f"🔄 Converting DIT model from WanModel to WanModel_mv for {wan_type}")
    ...
    def __call__(
        ...
        # 提取RGB和Heatmap的latents用于旋转和夹爪预测
