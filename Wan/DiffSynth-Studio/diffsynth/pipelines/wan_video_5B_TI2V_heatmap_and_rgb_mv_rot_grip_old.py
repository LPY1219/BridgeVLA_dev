...
from ..models.wan_video_dit_mv_rot_grip import WanModel_rot_grip, RMSNorm, sinusoidal_embedding_1d
...
# 用于对齐RGB和Heatmap latent的分布
# 基于VAE编码统计得出（见examples/wanvideo/model_inference/scaling_results/）
# ============================================================
# TODO: 根据实际测试结果修改这个值
HEATMAP_LATENT_SCALE_FACTOR = 1.0 # 默认不缩放
# HEATMAP_LATENT_SCALE_FACTOR = 2.734375  # 默认不缩放

def set_heatmap_scale_factor(scale: float):
    """
    设置Heatmap latent的缩放因子

    Args:
        scale: 缩放因子（建议值见scaling_results/）
    """
...
def convert_wan_model_to_rot_grip(wan_model, use_dual_head: bool = False, rotation_bins: int = 72, num_views: int = 3):
    """
    将 WanModel 转换为 WanModel_rot_grip，保留所有权重

    Args:
        wan_model: 原始的 WanModel 实例
        use_dual_head: 是否使用dual head模式
        rotation_bins: rotation离散化的bin数量（默认72，对应5度分辨率）
        num_views: 视图数量（默认3）

    Returns:
        WanModel_rot_grip 实例，包含所有原始权重
    """
    # 获取原始模型的配置
    config = {
        'dim': wan_model.dim,
    ...
    if hasattr(wan_model, 'head') and wan_model.head is not None:
        ...
        config['out_dim'] = 48  # hardcode default value

    # text_dim 从 text_embedding 获取
    if hasattr(wan_model, 'text_embedding'):
        config['text_dim'] = wan_model.text_embedding[0].in_features
    else:
        config['text_dim'] = 4096  # 默认值

    print(f"    Creating WanModel_rot_grip with config: dim={config['dim']}, in_dim={config['in_dim']}, "
          f"out_dim={config['out_dim']}, num_layers={config.get('num_layers', 'N/A')}, "
          f"use_dual_head={use_dual_head}, rotation_bins={rotation_bins}, num_views={num_views}")

    # 创建新的 WanModel_rot_grip 实例
    wan_model_mv = WanModel_rot_grip(**config)

    # 复制所有权重
    original_state_dict = wan_model.state_dict()

    # 如果使用 dual head，需要特殊处理 head 权重  # 这里其实有点多余，因为WanModel_mv 本身也支持dual_head,且为dual head时已经将head.head.weight给删掉了
    if use_dual_head and 'head.head.weight' in original_state_dict:
        ...
        for key, value in original_state_dict.items():
            if not key.startswith('head.'):
                ...
                # 将 head 权重复制到 head_rgb 和 head_heatmap
        ...
        print("    ✓ Converted to WanModel_rot_grip with dual head (head weights duplicated)")
        print("    ⚠️  Rotation and gripper heads are randomly initialized")
    else:
        # 直接加载所有权重
        missing_keys, unexpected_keys = wan_model_mv.load_state_dict(original_state_dict, strict=False)
        if len(missing_keys) > 0:
            # 区分rotation/gripper head的missing keys和其他missing keys
            rot_grip_missing = [k for k in missing_keys if k.startswith('head_rot.') or k.startswith('head_grip.')]
            other_missing = [k for k in missing_keys if not (k.startswith('head_rot.') or k.startswith('head_grip.'))]

            if rot_grip_missing:
                print(f"    ⚠️  Rotation and gripper heads are randomly initialized ({len(rot_grip_missing)} keys)")
            if other_missing:
                print(f"    ⚠️  Missing keys in WanModel_rot_grip: {len(other_missing)} keys")
                if len(other_missing) <= 10:
                    for key in other_missing:
                        print(f"      - {key}")
        if len(unexpected_keys) > 0:
            print(f"    ⚠️  Unexpected keys from WanModel: {len(unexpected_keys)} keys")
        if len(missing_keys) == 0 and len(unexpected_keys) == 0:
            print("    ✓ Successfully converted to WanModel_rot_grip (all weights transferred)")
        elif len([k for k in missing_keys if not (k.startswith('head_rot.') or k.startswith('head_grip.'))]) == 0:
            print("    ✓ Successfully converted to WanModel_rot_grip (all weights transferred, rot/grip heads randomly initialized)")
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.dit: WanModel_rot_grip = None
        self.dit2: WanModel_rot_grip = None
    ...
    def training_loss(self, **inputs):
        ...
        # model_fn 现在返回字典，包含latents, rotation_logits, gripper_logits
        output = self.model_fn(**inputs, timestep=timestep)

        # 如果output是字典（包含rotation和gripper预测）
        if isinstance(output, dict):
            noise_pred = output['latents']
            rotation_logits = output.get('rotation_logits', None)  # (b, f, 216) - 每个时间步一个预测（已聚合视图和空间）
            gripper_logits = output.get('gripper_logits', None)    # (b, f, 2) - 每个时间步一个预测（已聚合视图和空间）
        else:
            # 向后兼容：如果output是tensor，说明没有rotation和gripper预测
            noise_pred = output
        ...
        if not hasattr(self, '_training_loss_debug_printed'):
            ...
            if local_rank == 0:
                ...
                if rotation_logits is not None:
                    print(f"[DEBUG] rotation_logits.shape={rotation_logits.shape} (b, f, 216) - f is future_frames (num_frames-1)")
                    if 'rotation_targets' in inputs:
                        print(f"[DEBUG] rotation_targets.shape={inputs['rotation_targets'].shape}")
                if gripper_logits is not None:
                    print(f"[DEBUG] gripper_logits.shape={gripper_logits.shape} (b, f, 2) - f is future_frames (num_frames-1)")
                    if 'gripper_targets' in inputs:
                        print(f"[DEBUG] gripper_targets.shape={inputs['gripper_targets'].shape}")
            self._training_loss_debug_printed = True

        # 计算整体 loss (diffusion loss)
        loss = torch.nn.functional.mse_loss(noise_pred.float(), training_target.float())
        loss = loss * self.scheduler.training_weight(timestep)

        # 计算 RGB loss（前一半通道）和 Heatmap loss（后一半通道）
        ...
        loss_rgb = torch.nn.functional.mse_loss(noise_pred_rgb.float(), training_target_rgb.float())
        loss_rgb = loss_rgb * self.scheduler.training_weight(timestep)

        loss_heatmap = torch.nn.functional.mse_loss(noise_pred_heatmap.float(), training_target_heatmap.float())
        loss_heatmap = loss_heatmap * self.scheduler.training_weight(timestep)

        # 计算 Rotation loss (交叉熵损失) - 分别返回roll, pitch, yaw
        loss_roll = None
        loss_pitch = None
        loss_yaw = None
        loss_rotation = None
        if rotation_logits is not None and 'rotation_targets' in inputs:
            rotation_targets = inputs['rotation_targets']  # Expected: (b, seq_len, 3)

            # Debug: 打印实际形状
            import os
            local_rank = int(os.environ.get("LOCAL_RANK", 0))
            if local_rank == 0 and not hasattr(self, '_rotation_shape_debug_printed'):
                print(f"[DEBUG] rotation_targets actual shape: {rotation_targets.shape}")
                print(f"[DEBUG] rotation_logits shape: {rotation_logits.shape}")
                print(f"[DEBUG] rotation_targets dtype: {rotation_targets.dtype}")
                print(f"[DEBUG] rotation_targets ndim: {rotation_targets.ndim}")
                print(f"[DEBUG] Note: rotation_logits has been upsampled from VAE-compressed frames to future frames only (excluding start frame)")
                print(f"[DEBUG] Expected: rotation_logits frames = num_frames - 1 = future frames, matching rotation_targets")
                if rotation_targets.ndim == 2:
                    print(f"[DEBUG] rotation_targets will be unsqueezed to: (1, {rotation_targets.shape[0]}, {rotation_targets.shape[1]})")
                self._rotation_shape_debug_printed = True

            # 如果 rotation_targets 是 (f, 3) 需要添加 batch 维度
            if rotation_targets.ndim == 2:
                # 从 (f, 3) 扩展到 (1, f, 3)
                rotation_targets = rotation_targets.unsqueeze(0)

            # rotation_logits: (b, f, 216) = (b, f, 72*3) where f is future_frames (num_frames-1)
            # rotation_targets: (b, f, 3) - 每个元素是bin index, f is future_frames
            # 需要分别对roll, pitch, yaw计算交叉熵
            num_bins = rotation_logits.shape[-1] // 3  # 72

            # 重塑logits
            # logits: (b, f, 216) -> (b, f, 3, 72)
            rotation_logits_reshaped = rotation_logits.view(rotation_logits.shape[0], rotation_logits.shape[1], 3, num_bins)

            # 对roll, pitch, yaw分别计算损失
            loss_roll = torch.nn.functional.cross_entropy(
                rotation_logits_reshaped[:, :, 0, :].reshape(-1, num_bins),  # (b*f, 72)
                rotation_targets[:, :, 0].reshape(-1).long(),  # (b*f,)
                reduction='mean'
            )
            loss_pitch = torch.nn.functional.cross_entropy(
                rotation_logits_reshaped[:, :, 1, :].reshape(-1, num_bins),
                rotation_targets[:, :, 1].reshape(-1).long(),
                reduction='mean'
            )
            loss_yaw = torch.nn.functional.cross_entropy(
                rotation_logits_reshaped[:, :, 2, :].reshape(-1, num_bins),
                rotation_targets[:, :, 2].reshape(-1).long(),
                reduction='mean'
            )

            # 总的rotation loss是三个的平均
            loss_rotation = (loss_roll + loss_pitch + loss_yaw) / 3.0

        # 计算 Gripper loss (交叉熵损失)
        loss_gripper = None
        if gripper_logits is not None and 'gripper_targets' in inputs:
            gripper_targets = inputs['gripper_targets']  # Expected: (b, f) or (f,) where f is future_frames (num_frames-1)

            # Debug: 打印实际形状
            import os
            local_rank = int(os.environ.get("LOCAL_RANK", 0))
            if local_rank == 0 and not hasattr(self, '_gripper_shape_debug_printed'):
                print(f"[DEBUG] gripper_targets actual shape: {gripper_targets.shape}")
                print(f"[DEBUG] gripper_logits shape: {gripper_logits.shape}")
                self._gripper_shape_debug_printed = True

            # 如果 gripper_targets 是 (f,) 需要添加 batch 维度
            if gripper_targets.ndim == 1:
                # 从 (f,) 扩展到 (1, f)
                gripper_targets = gripper_targets.unsqueeze(0)

            # gripper_logits: (b, f, 2) where f is future_frames (num_frames-1)
            # gripper_targets: (b, f) where f is future_frames
            loss_gripper = torch.nn.functional.cross_entropy(
                gripper_logits.reshape(-1, 2),  # (b*f, 2)
                gripper_targets.reshape(-1).long(),  # (b*f,)
                reduction='mean'
            )

        # 计算总的 loss (diffusion loss + rotation loss + gripper loss)
        # 可以通过权重来平衡不同任务的重要性
        total_loss = loss  # 先从 diffusion loss 开始

        # 添加 rotation loss（如果存在）
        if loss_rotation is not None:
            rotation_weight = 1.0  # 可以根据需要调整权重
            total_loss = total_loss + rotation_weight * loss_rotation

        # 添加 gripper loss（如果存在）
        if loss_gripper is not None:
            gripper_weight = 1.0  # 可以根据需要调整权重
            total_loss = total_loss + gripper_weight * loss_gripper

        # Debug打印分离后的loss（仅在主进程打印一次）
        if not hasattr(self, '_loss_split_debug_printed'):
            ...
            if local_rank == 0:
                ...
                print(f"[DEBUG] loss_diffusion={loss.item():.4f}, loss_rgb={loss_rgb.item():.4f}, loss_heatmap={loss_heatmap.item():.4f}")
                if loss_rotation is not None:
                    print(f"[DEBUG] loss_rotation={loss_rotation.item():.4f}")
                    print(f"[DEBUG]   loss_roll={loss_roll.item():.4f}, loss_pitch={loss_pitch.item():.4f}, loss_yaw={loss_yaw.item():.4f}")
                if loss_gripper is not None:
                    print(f"[DEBUG] loss_gripper={loss_gripper.item():.4f}")
                print(f"[DEBUG] total_loss={total_loss.item():.4f}")
            self._loss_split_debug_printed = True

        result = {
            "loss": total_loss,  # 用于梯度反传的总 loss
            "loss_diffusion": loss,  # 单独保存 diffusion loss 用于监控
            "loss_rgb": loss_rgb,
            "loss_heatmap": loss_heatmap
        }

        # 添加rotation相关的loss和logits/targets（用于计算准确率）
        if loss_rotation is not None:
            result["loss_rotation"] = loss_rotation
            result["loss_roll"] = loss_roll
            result["loss_pitch"] = loss_pitch
            result["loss_yaw"] = loss_yaw
            # 添加logits和targets以便在训练循环中计算准确率
            result["rotation_logits"] = rotation_logits
            result["rotation_targets"] = inputs['rotation_targets']

        # 添加gripper loss和logits/targets（用于计算准确率）
        if loss_gripper is not None:
            result["loss_gripper"] = loss_gripper
            # 添加logits和targets以便在训练循环中计算准确率
            result["gripper_logits"] = gripper_logits
            result["gripper_targets"] = inputs['gripper_targets']
    ...
    def from_pretrained(
        ...
        rotation_bins: int = 72,
    ):
        if tokenizer_config is None:
            # 自动检测根路径
            if os.path.exists("/share/project/lpy/huggingface"):
                base_path = "/share/project/lpy/huggingface"
            elif os.path.exists("/data/lpy/huggingface"):
                base_path = "/data/lpy/huggingface"
            else:
                raise RuntimeError("Cannot find huggingface model directory in either /share/project/lpy/huggingface or /data/lpy/huggingface")

            if wan_type =="5B_TI2V":
                tokenizer_config = ModelConfig(path=f"{base_path}/Wan_2_2_TI2V_5B/google/umt5-xxl")
            elif wan_type =="5B_TI2V_RGB_HEATMAP" or wan_type=="5B_TI2V_RGB_HEATMAP_MV" or wan_type== "5B_TI2V_RGB_HEATMAP_MV_ROT_GRIP":
        ...
        # Download and load models (使用支持自动维度适配的 ModelManager)
        ...
        dit = model_manager.fetch_model("wan_video_dit", index=2) # 不知道是否需要修改？

        # Convert WanModel to WanModel_rot_grip for ROT_GRIP support
        # When wan_type ends with "_ROT_GRIP", we need to use WanModel_rot_grip which has rotation and gripper prediction
        if wan_type and wan_type.endswith("_ROT_GRIP"):
            print(f"🔄 Converting DIT model from WanModel to WanModel_rot_grip for {wan_type}")
            if isinstance(dit, list):
                converted_dit_list = []
                for single_dit in dit:
                    if single_dit is not None and type(single_dit).__name__ == 'WanModel':
                        converted_dit = convert_wan_model_to_rot_grip(single_dit, use_dual_head, rotation_bins, num_views=3) # hardcode
                ...
                if dit is not None and type(dit).__name__ == 'WanModel':
                    dit = convert_wan_model_to_rot_grip(dit, use_dual_head, rotation_bins, num_views=3) # hardcode
        ...
        if os.path.exists(config_path):
            ...
            if pipe.dit and hasattr(pipe.dit, 'in_dim'):
                ...
                if need_adaptation:
                    print(f"⚠️  DIT dimension mismatch detected:")
                    ...
                    # 只加载 DIT 的 checkpoint（model_configs[0]），不包括 text encoder 和 VAE
                    ...
                    # 应用 state_dict 转换（与 model_manager 加载时使用相同的转换逻辑）
                    ...
                    # converted_state_dict 即为要传递给 adapt_pretrained_weights 的 DIT 参数
                    dit_state_dict = converted_state_dict

                    # 更新模型的维度并重新初始化相关层（保持dtype一致）
                    ...
                    if  not use_dual_head:
                        if config_out_dim: # 当为dual head 的时候，其实就不应该初始化这里
                            from ..models.wan_video_dit import Head
                            eps = config.get("eps", 1e-6)  # 从config获取eps，默认1e-6
                    ...
                    if hasattr(pipe.dit, 'adapt_pretrained_weights'):
                        ...
                        if adaptation_result['size_mismatched_keys']:
                            ...
                            for key in adaptation_result['size_mismatched_keys'][:5]:  # 只显示前5个
                                print(f"      - {key}")
    ...
    def __call__(
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if cfg_scale != 1.0:  #需要等于1.0 ，因为训练的时候是1.0
                if cfg_merge:
            ...
            if "first_frame_latents" in inputs_shared and  "first_frame_latents_rgb" in inputs_shared:
                ...
                inputs_shared["latents"][:, :, 0:1] = first_frame_latents_fused  # 注意这里的形状，看看是不是要修改一下。
        ...
        video_rgb_1 = self.vae.decode(inputs_shared["latents"][0:1,:num_channels//2], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        ...
        video_heatmap_1 = self.vae.decode(heatmap_latents, device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_heatmap_1 = self.vae_output_to_video(video_heatmap_1)
        
        
        video_rgb_2 = self.vae.decode(inputs_shared["latents"][1:2,:num_channels//2], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_rgb_2 = self.vae_output_to_video(video_rgb_2)
        # Apply inverse scaling to heatmap latents before decoding
        heatmap_latents = inputs_shared["latents"][1:2,num_channels//2:]
        if HEATMAP_LATENT_SCALE_FACTOR != 1.0:
            heatmap_latents = heatmap_latents / HEATMAP_LATENT_SCALE_FACTOR
        video_heatmap_2 = self.vae.decode(heatmap_latents, device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_heatmap_2 = self.vae_output_to_video(video_heatmap_2)
        
        video_rgb_3 = self.vae.decode(inputs_shared["latents"][2:3,:num_channels//2], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_rgb_3 = self.vae_output_to_video(video_rgb_3)
        # Apply inverse scaling to heatmap latents before decoding
        heatmap_latents = inputs_shared["latents"][2:3,num_channels//2:]
        if HEATMAP_LATENT_SCALE_FACTOR != 1.0:
            heatmap_latents = heatmap_latents / HEATMAP_LATENT_SCALE_FACTOR
        video_heatmap_3 = self.vae.decode(heatmap_latents, device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_heatmap_3 = self.vae_output_to_video(video_heatmap_3)    
        
        
        self.load_models_to_device([])

        return [video_heatmap_1,video_heatmap_2,video_heatmap_3],[video_rgb_1,video_rgb_2,video_rgb_3]
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("height", "width", "num_frames", "seed", "rand_device", "vace_reference_image","num_view"))

    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, vace_reference_image,num_view):
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_video", "noise", "tiled", "tile_size", "tile_stride", "vace_reference_image","input_video_rgb"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image,input_video_rgb):
        if input_video is None:
            return {"latents": noise,"rgb_latents": None}
        pipe.load_models_to_device(["vae"])

        # 处理多视角输入: input_video shape is (T, num_views, 3, H, W)
        # 需要将每个视角分别处理，然后合并
        if isinstance(input_video, list) and len(input_video) > 0 and isinstance(input_video[0], list):
            ...
            for view_idx in range(num_views):
                ...
                view_video = pipe.preprocess_video(view_frames)
                view_latents = pipe.vae.encode(view_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)

                # Apply scaling to heatmap latents during training
                if pipe.scheduler.training and HEATMAP_LATENT_SCALE_FACTOR != 1.0:
                    view_latents = view_latents * HEATMAP_LATENT_SCALE_FACTOR

                all_view_latents.append(view_latents)

            # 合并所有视角的latents, 沿batch维度拼接: (num_views, C, T, H, W)
            ...
            # Apply scaling to heatmap latents during training
            if pipe.scheduler.training and HEATMAP_LATENT_SCALE_FACTOR != 1.0:
                input_latents = input_latents * HEATMAP_LATENT_SCALE_FACTOR
        ...
        if vace_reference_image is not None:
            ...
            if isinstance(vace_reference_image, list):
                ...
                for view_idx in range(num_views):
                    ...
                    # Apply scaling to heatmap reference latents during training
                    if pipe.scheduler.training and HEATMAP_LATENT_SCALE_FACTOR != 1.0:
        ...
        if input_video_rgb is not None:
            # 处理RGB视频 (多视角): input_video_rgb shape is (T, num_views, 3, H, W)
            if isinstance(input_video_rgb, list) and len(input_video_rgb) > 0 and isinstance(input_video_rgb[0], list):
                ...
                for view_idx in range(num_views):
                    ...
                    view_rgb_video = pipe.preprocess_video(view_rgb_frames)
                    view_rgb_latents = pipe.vae.encode(view_rgb_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
                ...
                input_video_rgb = pipe.preprocess_video(input_video_rgb)
                input_latents_rgb = pipe.vae.encode(input_video_rgb, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)

            input_latents = torch.concat([input_latents_rgb, input_latents], dim=1) # 沿着channel维度拼接

        if pipe.scheduler.training:
            return {"latents": noise, "input_latents": input_latents} # inputs["latents"]和inputs["noise"]共享存储
        else:
...
class WanVideoUnit_ImageEmbedderFused(PipelineUnit):
    ...
    def __init__(self):
        super().__init__(
            input_params=("input_image", "latents", "height", "width", "tiled", "tile_size", "tile_stride","input_image_rgb"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, input_image, latents, height, width, tiled, tile_size, tile_stride,input_image_rgb):
        if input_image is None or not pipe.dit.fuse_vae_embedding_in_latents:
            return {}
        pipe.load_models_to_device(self.onload_model_names)

        # Handle multi-view images (List[PIL.Image])
        ...
        latents[:, :, 0: 1] = z_fused
        return {"latents": latents, "fuse_vae_embedding_in_latents": True, "first_frame_latents": z, "first_frame_latents_rgb": z_rgb}
...
class TeaCache:
    ...
    def check(self, dit: WanModel_rot_grip, x, t_mod):
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    # 预测rotation和gripper（heads内部会聚合多视图特征和空间特征，并进行时间上采样）
    # 传递num_frames-1因为rotation/gripper只预测future frames（不包括start frame）
    # num_frames = 1(start) + N(future), 所以future frames = num_frames - 1
    num_future_frames = num_frames - 1 if num_frames is not None else None
    rotation_logits = dit.head_rot(features, t, (f, h, w), num_frames_orig=num_future_frames)  # (b, f_future, 216)
    gripper_logits = dit.head_grip(features, t, (f, h, w), num_frames_orig=num_future_frames)  # (b, f_future, 2)
    ...
    if reference_latents is not None:
        ...
        f -= 1
    latents = dit.unpatchify(x, (f, h, w))
    return {
        'latents': latents,
        'rotation_logits': rotation_logits,
        'gripper_logits': gripper_logits
