...
class WanVideoPipeline(BasePipeline):
    ...
    def from_pretrained(
        ...
        # 🔧 强制设置 fuse_vae_embedding_in_latents 和 seperated_timestep 为 True
        # 对于 Wan2.2-TI2V-5B (HISTORY pipeline)，这两个属性必须为 True 才能正确生成 4D t_mod
        # 这是因为 WanVideoPostUnit_MV_History 会检查 pipe.dit.fuse_vae_embedding_in_latents
        # 如果为 False，unit 会跳过处理，导致 model_fn 使用 3D t_mod (has_seq=False)
    ...
    def _decode_with_history(self, latents, num_history_frames, tiled, tile_size, tile_stride):
        """
        根据 num_history_frames 解码 latents 为视频帧
        参考训练代码 mv_rot_grip_vae_decode_feature_3.py 中的 _encode_and_decode_single_view_with_history

        当 num_history_frames=2 时，latents 的结构是：
        - latents[:, :, 0:1] - 第1帧单独编码的 latent
        - latents[:, :, 1:] - 第2帧+剩余帧正常编码的 latent

        需要分别解码并 concat，确保输出帧数与输入帧数一致。

        Args:
            latents: (batch=1, c, t_compressed, h, w) - 单个视角的 latent
        ...
        Returns:
            video: 解码后的视频 tensor (batch, C, T, H, W)
        """
        # latents shape: (1, c, t, h, w) - 来自 inputs_shared["latents"][view_idx:view_idx+1, ...]
        # vae.decode 期望 list of (c, t, h, w)
    ...
    def __call__(
        ...
        inputs_shared = {
            ...
            "use_dual_head": use_dual_head,
            "num_view": num_view,
            # 🔧 从dit模型获取fuse_vae_embedding_in_latents，确保传递给model_fn_wan_video
...
class WanVideoUnit_ShapeChecker(PipelineUnit):
    ...
    对于多历史帧模式，帧数检查规则不同：
    - num_history_frames=1: 需要 num_frames % 4 == 1 (原始规则)
    - num_history_frames=2: 第1帧单独 + (第2帧+剩余帧)正常编码
      需要 (num_frames - 1) % 4 == 1，即 num_frames % 4 == 2
    - num_history_frames=1+4N: 需要 num_frames % 4 == 1 (原始规则)
    """
    def __init__(self):
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, vace_reference_image, num_view, num_history_frames):
        # 计算latent时间维度长度，需要考虑 num_history_frames
        # 编码规则：
        ...
        if num_cond_frames == 2:
            # 第1帧单独 → 1个latent
            # 第2帧+剩余帧作为新视频 → ((num_frames-1) - 1) // 4 + 1 个latent
            remaining_frames = num_frames - 1  # 从第2帧开始的帧数
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    支持 num_history_frames 参数，控制前几帧作为条件。

    编码规则（与VAE预训练一致）：
    - num_history_frames=1: 正常编码（第一帧单独，后续每4帧一组）
    - num_history_frames=2: 前2帧分别单独编码后concat，其余正常编码
    - num_history_frames=1+4N: 正常编码（VAE自动处理）
    """
    def __init__(self):
        super().__init__(
    ...
    def _encode_video_with_history(self, pipe, video_frames, num_history_frames, tiled, tile_size, tile_stride, apply_scale=False):
        ...
        Args:
            pipe: Pipeline实例
            video_frames: 视频帧列表 (List[PIL.Image])
            num_history_frames: 历史帧数量
            tiled, tile_size, tile_stride: VAE编码参数
            apply_scale: 是否应用heatmap缩放因子

        Returns:
            编码后的latents

        编码规则：
        - num_history_frames=1: 正常编码
        - num_history_frames=2: 第1帧单独编码，第2帧+剩余帧作为新视频正常编码
        - num_history_frames=1+4N: 正常编码
        """
        ...
        if num_history_frames == 2 and num_frames >= 2:
            ...
            remaining_frames = video_frames[1:]  # 从第2帧开始
...
def compute_num_condition_latents(num_history_frames):
    ...
    - num_history_frames=2: 2个latent（每帧单独）
    ...
    Returns:
        num_condition_latents: 条件latent数量
    """
...
class WanVideoUnit_ImageEmbedderFused(PipelineUnit):
    ...
    支持多帧历史图像输入（按照VAE原始方式编码）:
    - input_image: List[PIL.Image] - 单帧条件（向后兼容）
    - input_images: List[List[PIL.Image]] - 多帧历史条件，每个元素是一帧的多视角列表
    - input_image_rgb: List[PIL.Image] - 单帧RGB条件
    - input_images_rgb: List[List[PIL.Image]] - 多帧历史RGB条件

    历史帧编码规则（保持与VAE预训练一致）：
    - num_history_frames=1: 1帧 → 1个条件latent
    - num_history_frames=2: 2帧分别单独编码 → 2个条件latent
    - num_history_frames=1+4N: 第一帧单独 + 后续每4帧一组 → (1+N)个条件latent

    条件latent会注入到latents的前num_condition_latents个时间步。
    """
    def __init__(self):
        super().__init__(
    ...
    def process(self, pipe: WanVideoPipeline, input_image, input_images, latents, height, width, tiled, tile_size, tile_stride, input_image_rgb, input_images_rgb, num_history_frames):
        ...
        if use_multi_frame:
            # 多帧历史模式: input_images 是 List[List[PIL.Image]] - num_history x num_views
            ...
            for view_idx in range(num_views):
                if num_history == 2:
                    ...
                    # 在时间维度上concat: (1, C, 1, H, W) + (1, C, 1, H, W) -> (1, C, 2, H, W)
                    ...
                    # 1+4N 帧：收集所有历史帧，拼接为视频格式，让VAE自动处理
                    view_heatmap_frames = []
                    view_rgb_frames = []
                    ...
                    # 将多帧拼接为视频格式 (3, T, H, W) - T为历史帧数
                    # stack在第1维（时间维度）得到 (3, num_history, H, W)
                    ...
                    # VAE编码 - 使用原始VAE逻辑（第一帧单独，后续每4帧一组）
                    # 这会自动产生正确数量的latent时间步
        ...
        # 注入条件latent到latents的前num_condition_latents个时间步
        latents[:, :, 0:num_condition_latents] = z_fused

        return {
            "latents": latents,
            "fuse_vae_embedding_in_latents": True,
            "condition_latents": z,  # 重命名以更准确反映其含义
            "condition_latents_rgb": z_rgb,
            "num_condition_latents": num_condition_latents  # 传递条件latent数量
        }
...
def model_fn_wan_video(
    ...
    use_dual_head:bool=False,
    num_condition_latents: int = 1,  # 条件latent数量，前num_condition_latents个latent的timestep设为0
    **kwargs,
):
    ...
    # Timestep
    # 当使用多个条件latent时，前num_condition_latents个latent的timestep都设为0
    # Debug: 检查 seperated_timestep 和 fuse_vae_embedding_in_latents
    ...
    if dit.seperated_timestep and fuse_vae_embedding_in_latents:
        ...
        num_pred_latents = latents.shape[2] - num_condition_latents # 有问题
        timestep = torch.concat([
            # 条件latent的timestep全部设为0
            torch.zeros((num_condition_latents, spatial_size), dtype=latents.dtype, device=latents.device),
            # 预测latent的timestep设为实际timestep
