...
class RobotTrajectoryDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            {
                'rgb_image': torch.Tensor,      # 首帧投影后的多视角RGB图像 (num_views, 3, H, W)
                'rgb_sequence': torch.Tensor,   # 未来帧的多视角RGB图像序列 (T, num_views, 3, H, W)
                ...
                'future_poses': torch.Tensor,   # 未来poses (T, 3)
        ...
        rgb_image = rgb_image[0, :, :, :, 3:]  # (3,256,256,3)
        ...
        for view_idx in range(num_views):
            ...
            if view_img.shape[:2] != self.image_size:
                from PIL import Image
                rgb_pil = Image.fromarray((view_img * 255).astype(np.uint8))
                rgb_pil = rgb_pil.resize((self.image_size[1], self.image_size[0]))
                view_img = np.array(rgb_pil) / 255.0
            resized_rgb_views.append(view_img)
        rgb_image = np.stack(resized_rgb_views, axis=0)  # (num_views, H, W, 3)
        ...
        for future_pcd, future_rgb in zip(processed_future_pcds, processed_future_rgbs):
            ...
            )  # (1, num_views, 256, 256, 6)
            future_rgb_image = future_rgb_image[0, :, :, :, 3:]  # (num_views, 256, 256, 3)
            ...
            # 调整每个视角的图像尺寸
            resized_future_views = []
            for view_idx in range(num_views):
                view_img = future_rgb_image[view_idx]  # (H, W, 3)
                if view_img.shape[:2] != self.image_size:
                    from PIL import Image
                    rgb_pil = Image.fromarray((view_img * 255).astype(np.uint8))
                    rgb_pil = rgb_pil.resize((self.image_size[1], self.image_size[0]))
                    view_img = np.array(rgb_pil) / 255.0
                resized_future_views.append(view_img)
            future_rgb_image = np.stack(resized_future_views, axis=0)  # (num_views, H, W, 3)

            # 转换为tensor并permute: (num_views, 3, H, W)
            future_rgb_tensor = torch.from_numpy(future_rgb_image).float().permute(0, 3, 1, 2)
        ...
        rgb_tensor = torch.from_numpy(rgb_image).float().permute(0, 3, 1, 2)
