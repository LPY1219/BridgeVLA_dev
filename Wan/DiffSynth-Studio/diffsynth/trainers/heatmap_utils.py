...
def prepare_heatmap_data_for_wan_5B_TI2V_RGB_HEATMAP_MULTIVIEW_ROT_GRIP(
    ...
    Args:
        ...
        metaworld: 是否为Metaworld数据集（影响gripper离散化方式）
    ...
    if not metaworld:
        ...
        # metaworld数据集，gripper state是连续值[-1, 1]，需要量化为21类
        # Process start_gripper (continuous value in [-1,1], quantize into 21 classes)
        if start_gripper_state is not None:
            # Convert to float
            if isinstance(start_gripper_state, bool):
                val = float(start_gripper_state)
            else:
                val = float(start_gripper_state)

            # Quantize to 0..20 (21 classes at step 0.1)
            cls = round((val + 1.0) / 0.1)
            cls = max(0, min(20, cls))  # safety clipping

            start_gripper = torch.tensor(cls, dtype=torch.long)
            data['start_gripper'] = start_gripper


        # Process future_gripper_states (tensor or list of float values)
        if future_gripper_states is not None:
            # Convert to tensor of floats
            if torch.is_tensor(future_gripper_states):
                vals = future_gripper_states.float()
            else:
                vals = torch.tensor(future_gripper_states, dtype=torch.float32)

            # Quantize each value
            cls = torch.round((vals + 1.0) / 0.1).long()
            cls = torch.clamp(cls, min=0, max=20)

            data['gripper_targets'] = cls  # shape (T,)
    ...
    if not metaworld:
        ...
        if future_gripper_states is not None and start_gripper_state is not None:
            # Convert start to float, then quantize to class
            start_val = float(start_gripper_state)
            start_cls = round((start_val + 1.0) / 0.1)
            start_cls = max(0, min(20, start_cls))
            start_cls = int(start_cls)

            # Convert future states to tensor of floats
            if torch.is_tensor(future_gripper_states):
                vals = future_gripper_states.float()
            else:
                vals = torch.tensor(future_gripper_states, dtype=torch.float32)

            # Quantize future values
            future_cls = torch.round((vals + 1.0) / 0.1).long()
            future_cls = torch.clamp(future_cls, 0, 20)

            # 0 = same as first frame class, 1 = different
            gripper_change_targets = (future_cls != start_cls).long()

            data['gripper_change_targets'] = future_cls  # (T,)
    return data
