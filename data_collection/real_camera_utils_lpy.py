...
def get_cam_extrinsic(type):
    if type == "3rd_1":
        ...
        trans=np.array([0.635715492673955, -0.048898669027370056, 1.2679230550039957])
        quat=np.array([ -0.7019261185534902, -0.7053580236921018,0.06362612905991612,0.07564058570936025]) # x y z w
    elif type == "3rd_2":
        # TODO: 填充第二个第三视角相机的实际外参
        trans=np.array([0.41185562602691036, 0.6730596164795135,  0.31856636560103526])
        quat=np.array([ -0.008074836643020937, -0.807607210587767,  0.5888584624088505,  0.036731108806098006]) # x y z w
        ...
        trans=np.array([1.1313142792439193, -0.05215042562872227,  0.23119278984261538])
        quat=np.array([ -0.5133577394053142, -0.5152325732778219,0.49100817059711893,0.47948952358729374]) # x y z w
    else:
