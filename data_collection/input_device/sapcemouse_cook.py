...
class FrankaSpacemouse(mp.Process):
    ...
    def __init__(self, 
        ...
        # 按钮功能映射（已禁用，所有自由度默认解锁）
        if button_functions is None:
            self.button_functions = {
                0: 'unused',  # 左键：未使用（所有自由度已默认启用）
                1: 'unused'   # 右键：未使用（所有自由度已默认启用）
            }
        else:
    ...
    def apply_control_logic(self, motion_state: np.ndarray,
        ...
        # 默认全部解锁：平移XYZ + 旋转RPY 全部启用
        rotation_enabled = True  # 旋转始终启用
        z_unlocked = True        # Z轴始终解锁
        coordinate_frame_index = 0 if self.coordinate_frame == 'world' else 1

        # 不再需要按钮控制，所有6个自由度默认启用
        # processed_motion 保持原样，不做任何置零操作
